...
The deciding factor is **whether two documents share a revision cycle** (move together), not whether they are in the same module.

- **Same revision cycle** (e.g., files within `interface-contract/draft/v1.0-r9/`): relative paths are fine — they always move together.
- **Independent revision cycles** (e.g., `interface-contract/draft/v1.0-r9/` → `behavior/draft/v1.0-r1/`, or any cross-module reference): **do NOT use exact file path links**. Exact paths encode revision numbers that break every time the target is revised.
