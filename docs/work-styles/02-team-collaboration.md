...
**Definition:** A zombie agent is an agent that is alive (its process is running) but **unreachable** — the team leader cannot find or communicate with it. This happens when the team directory (`~/.claude/teams/<name>/`) is deleted while agents are still running, severing the communication channel permanently. An agent that is alive, has a team directory, and responds to messages is **NOT** a zombie — even if it was spawned in a previous session. Do not confuse "from a previous session" with "zombie."
