...
> `01-overview.md` in the interface-contract docs.

---

## 1. When Does hangul_ic_process() Return False?

When `hangul_ic_process()` returns `false`, libhangul rejected the key -- it is
not a valid jamo for the current keyboard layout. This occurs with:

- Punctuation (`.`, `,`, `!`, `?`, etc.)
- Certain number keys (depending on keyboard layout)
- Other characters libhangul does not recognize as jamo

Returning `false` does NOT mean "nothing happened." libhangul may still update
its internal commit and preedit buffers as a side effect (e.g., a syllable break
triggered by the rejected character).
...
The most common mistake is to skip buffer checks when `hangul_ic_process()`
returns `false`. libhangul may produce committed text even on a rejected key.
For example, if the composition state triggers a syllable break before rejecting
the character, `hangul_ic_get_commit_string()` will contain the completed
syllable.
...
3. Since `hangul_ic_process()` returned false:
   - Composition was non-empty ("ㅎ" in preedit), so flush via
     `hangul_ic_flush()` -- produces committed text "ㅎ".
...
If `hangul_ic_process()` returns `false` and no composition is in progress
(engine is empty), there is nothing to flush. The key is simply forwarded.
...
Some keys may trigger a syllable break (producing committed text via
`hangul_ic_get_commit_string()`) AND then be rejected (return `false`). In this
case:
...
This algorithm was verified by PoC (`poc/02-ime-ghostty-real/poc-ghostty-real.m`
lines 298-324).
