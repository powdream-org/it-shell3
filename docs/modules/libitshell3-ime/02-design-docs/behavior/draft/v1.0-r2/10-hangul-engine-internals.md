...
- **Date**: 2026-03-22
- **Scope**: Concrete implementation details of `HangulImeEngine` -- struct
  fields, engine mode dispatch, libhangul keyboard ID mapping, internal buffer
  layout, `setActiveInputMethod` internal step sequence, and Backspace jamo
  decomposition.

> This document describes **implementation internals** of the `HangulImeEngine`
> concrete type. For `ImeResult` and `KeyEvent` type definitions, see
> `02-types.md` in the interface-contract docs. For the `ImeEngine` vtable, see
> `03-engine-interface.md` in the interface-contract docs.
...
`EngineMode` is an internal optimization, NOT part of the public API. It
controls hot-path dispatch in `processKeyImpl`:

- **`direct`**: Key is mapped to ASCII and returned as `committed_text`. No
  libhangul involvement. Used for `"direct"` input method (English, Latin
  scripts).
- **`composing`**: Key is fed to `hangul_ic_process()` for jamo composition.
  Used for all `"korean_*"` input methods.
...
The engine maps canonical input method strings to libhangul keyboard IDs. This
mapping is internal to the engine -- no other component needs to know libhangul
keyboard IDs.
...
`ImeResult` fields (`committed_text`, `preedit_text`) are slices pointing into
fixed-size internal buffers owned by the `HangulImeEngine` instance:
...
- **256 bytes for committed text**: A single Korean syllable is 3 bytes UTF-8.
  The longest possible commit from one keystroke is a flushed syllable + a
  non-jamo character = ~6 bytes. 256 bytes is vastly oversized for safety.
- **64 bytes for preedit**: A single composing syllable is always exactly one
  character (3 bytes UTF-8). 64 bytes is vastly oversized for safety.

### 3.2 libhangul Memory Model Reference

This design mirrors libhangul's own memory model --
`hangul_ic_get_preedit_string()` returns an internal pointer invalidated by the
next `hangul_ic_process()` call. The engine copies libhangul's UCS-4 output into
its own UTF-8 buffers. Zero heap allocation per keystroke.
...
**Caller responsibility**: If the caller needs to retain the text across
multiple `processKey()` calls, it must copy the data. See
`02-integration-boundaries.md` in the `libitshell3` daemon design docs for the
daemon's consumption invariant.

---

## 4. setActiveInputMethod Internal Step Sequence

`setActiveInputMethod()` is the only input-method-switching method. Its internal
steps depend on whether the requested input method differs from the current one.
...
5. Return `ImeResult`:
   - If composition was active (flushed text is non-empty):
...
`preedit_changed` is `true` only when the preedit state actually transitions
(non-null to null from flushing). When the engine was already empty, preedit
remains null throughout -- no transition occurred.
...
**Rationale**: The user toggled to the same mode by mistake (or the framework
called it redundantly). Flushing would be a surprising side effect. This matches
fcitx5 (`InputMethodManager::setCurrentGroup()`) and ibus
...
strings from the canonical registry (Section 2). Receiving an unrecognized
string is a server bug.

### 4.4 hangul_ic_flush() Cleanup

`hangul_ic_flush()` alone is sufficient -- no need to call `hangul_ic_reset()`
after flush. Verified in libhangul source: `hangul_ic_flush()` calls
`hangul_buffer_clear()` which zeroes all jamo fields (`choseong`, `jungseong`,
`jongseong`) and clears the stack. After flush, `hangul_ic_is_empty()` returns
true.
...
The `processKeyImpl` function must handle the case where `hangul_ic_process()`
returns `false` (key rejected by libhangul). See
[11-hangul-ic-process-handling.md](11-hangul-ic-process-handling.md) for the
full algorithm.

---

## 6. Backspace and Jamo Decomposition

During active Korean composition, Backspace undoes the most recently entered
jamo rather than deleting the entire composing syllable. The engine calls
`hangul_ic_backspace()` directly and uses its return value as the dispatch
signal -- no pre-check with `hangul_ic_is_empty()` is needed.

### 6.1 `hangul_ic_backspace()` Return Value Semantics

- **Returns `true`**: A jamo was popped from the internal buffer. The key is
  consumed by the engine. Read the updated preedit from
  `hangul_ic_get_preedit_string()`.
- **Returns `false`**: The composition was already empty (no jamo to remove).
  The key is not consumed -- forward Backspace to the terminal.

Calling `hangul_ic_backspace()` on an empty context is safe -- it returns
`false` and does nothing. This makes a separate `hangul_ic_is_empty()` guard
redundant.

### 6.2 Jamo Stack Pop Order

libhangul's `HangulBuffer` maintains jamo positions (choseong, jungseong,
jongseong) with length counters. `hangul_ic_backspace()` decrements the most
recently filled position in reverse input order:

1. **jongseong** (tail consonant) -- removed first if present
2. **jungseong** (vowel) -- removed next
3. **choseong** (leading consonant) -- removed last

This is a literal stack pop on the internal jamo buffer, not a reconstruction
from the composed syllable code point.

### 6.3 Double-Tail Consonant Decomposition

Double-tail consonants (e.g., ㅂㅅ in "없") are stored as two jongseong entries
in the buffer. Backspace removes the second component first. For example,
Backspace on "없" removes ㅅ from the tail, yielding "업" (ㅂ remains as the
single tail consonant). A subsequent Backspace would then remove ㅂ, yielding
"어" (no tail consonant).

### 6.4 ImeResult Patterns

The two cases map directly to `ImeResult` construction:

- **`hangul_ic_backspace()` returns `true`**: Read updated preedit from
  `hangul_ic_get_preedit_string()`, convert UCS-4 to UTF-8 into the engine's
  internal preedit buffer.
  ...
  No `committed_text` is produced -- Backspace only modifies the in-progress
  composition, it never commits.

- **`hangul_ic_backspace()` returns `false`**: Composition was empty, forward
...
### 6.5 Concrete Example: "한" Decomposition

Starting from preedit "한" (ㅎ + ㅏ + ㄴ), the full Backspace sequence:

| Step | Action    | `hangul_ic_backspace()` | Preedit | Removed               |
| ---- | --------- | ----------------------- | ------- | --------------------- |
| 1    | Backspace | returns `true`          | "하"    | ㄴ (jongseong)        |
| 2    | Backspace | returns `true`          | "ㅎ"    | ㅏ (jungseong)        |
| 3    | Backspace | returns `true`          | (empty) | ㅎ (choseong)         |
| 4    | Backspace | returns `false`         | (empty) | — forward to terminal |

Steps 1–3 each produce `ImeResult` with `preedit_changed = true` and the updated
(or null) preedit. Step 4 produces `ImeResult` with `forward_key = Backspace`.

---

## 7. Session Persistence

`active_input_method` (string) is the only **engine-internal** field needed to
reconstruct a `HangulImeEngine` on session restore -- the server creates a new
engine with the saved `input_method` string. Composition state is never
persisted -- the engine always starts with empty composition. See
`02-integration-boundaries.md` in the `libitshell3` daemon design docs for the
full persistence and lifecycle details.
