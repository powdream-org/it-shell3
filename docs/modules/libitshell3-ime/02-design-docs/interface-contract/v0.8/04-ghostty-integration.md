...
ghostty integration — how `ImeResult` is consumed by the daemon to drive the key input and preedit pipeline (`key_encode.encode()`, `write(pty_fd)`, `overlayPreedit()`) — is defined in [daemon design doc 01 §4](../../libitshell3/02-design-docs/daemon/v0.3/01-internal-architecture.md) and [daemon design doc 02 §4](../../libitshell3/02-design-docs/daemon/v0.3/02-integration-boundaries.md#4-ime-integration-libitshell3-ime).
...
**Caller responsibility**: If the caller needs to retain the text across multiple `processKey()` calls, it must copy the data. See [daemon design doc 02 §4.6](../../libitshell3/02-design-docs/daemon/v0.3/02-integration-boundaries.md#46-critical-runtime-invariant) for the daemon's consumption invariant.
