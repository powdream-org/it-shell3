...
> **Status**: Draft v0.4 — Cross-review resolutions applied + identifier consensus applied.
> **Supersedes**: [v0.3/01-interface-contract.md](../v1.0-r3/01-interface-contract.md), [v0.2/01-interface-contract.md](../v1.0-r2/01-interface-contract.md), [v0.1/01-interface-contract.md](../v1.0-r1/01-interface-contract.md)
> **Date**: 2026-03-05
...
/// reset(), or setActiveInputMethod() on the SAME engine instance.
...
> **Naming convention**: Language-specific composition state constants MUST use a language prefix (`ko_`, `ja_`, `zh_`) to avoid collision across engines. For example, a future Japanese engine might define `ja_romaji_input`, `ja_kana_converting`, etc. The `empty` constant is the only language-agnostic value (no composition active) and carries no prefix. Note: the `ko_` prefix is used for composition states (engine-internal runtime state), NOT for input method identifiers (which use the full `"korean_"` prefix).
...
**Superseded by Appendix F**: Session persistence simplified to a single `input_method` field (e.g., `"korean_2set"`). No separate `active_language` or `layout_id`. See [F.6](#f6-session-persistence-simplified).
