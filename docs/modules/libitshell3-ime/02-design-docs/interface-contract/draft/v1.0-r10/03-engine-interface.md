...
**Decomposition boundary**: The `input_method` string flows unchanged from the
client through the server to the engine — no intermediate code examines or
transforms it for routing purposes. Only two entry points decompose the string
into engine-specific types (e.g., libhangul keyboard IDs): the engine
constructor (`init()`) and `setActiveInputMethod()`. All valid `input_method`
strings are enumerated in the Canonical Input Method Registry in
...
## 3. Per-Session Engine Architecture

Each session has one `ImeEngine` instance, shared across all panes in that
session. The engine maintains a single composition context — for Korean, this
means one `HangulInputContext` with one jamo stack. A consequence of this
single-context architecture is that at most one pane per session can have active
preedit at any time; the engine is pane-agnostic and does not enforce this
itself, but the constraint holds structurally because there is only one
composition buffer. Clients MAY rely on this invariant for rendering
optimization: when a preedit starts on one pane, any previously active preedit
on another pane within the same session has already been cleared.

## 4. MockImeEngine (For Testing)
