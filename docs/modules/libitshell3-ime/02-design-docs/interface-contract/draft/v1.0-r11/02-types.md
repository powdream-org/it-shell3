...
pub const KeyEvent = struct {
    ...
    /// classification" modifiers that affect key identity (letter case,
    ...
    /// Enter (0x28), Escape (0x29), Tab (0x2B), and Space (0x2C) are
    /// flush-triggering or forwarding keys, never composition input.
    /// Backspace (0x2A) triggers the IME undo handler, not the flush/forward path.
...
- `action` (press/release/repeat) added based on ghostty's
  ...
  protocol support. The IME engine ignores release events.
- `caps_lock` and `num_lock` are "input classification" modifiers. CapsLock is
  needed for direct/English mode character case resolution. NumLock is needed
  for numpad key classification. Wire-compatible: bits 3-4 were previously
  padding (always zero), now carry data.
- **Numpad keys and `isPrintablePosition()`**: Numpad keys (HID 0x54-0x63) are
  outside the printable range. The daemon sends all keycodes within
  HID_KEYCODE_MAX to `processKey()`. The engine forwards numpad keys
  unconditionally via `forward_key` — they never enter composition in any v1
  input method. NumLock state is preserved in Modifiers for wire losslessness
  but is not consumed by the v1 engine.
