...
**On session restore**: `HangulImeEngine.init(allocator, saved_input_method)`
with the persisted `input_method` string. No composition state is persisted —
the engine always starts with empty composition. See daemon doc 04 §8 for the
full session persistence schema.
...
consume(entry.session.focused_pane.pty, result)  // MUST consume before next engine call
entry.session.focused_pane = pane_b

// Inter-session tab switch (entry1 -> entry2)
result = entry1.session.ime_engine.deactivate()
consume(entry1.session.focused_pane.pty, result)
...
The single-threaded kqueue event loop (see internal architecture doc,
Resolution 2) provides implicit serialization. All key events and focus changes
are processed sequentially in the same thread. No locks, no mutexes, no data
races.
...
Mouse events bypass IME entirely. Mouse tracking is only active when the
terminal's DEC mode state has mouse reporting enabled (determined by
