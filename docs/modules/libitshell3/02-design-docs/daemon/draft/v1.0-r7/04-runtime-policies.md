...
- **Scope**: Connection limits, multi-client resize, health escalation, flow
  ...
  procedures, notification defaults, heartbeat policy, and silence detection
  timer
...
3), the daemon re-scans the remaining attached clients and selects the first
healthy client found. If multiple healthy clients exist, the daemon picks the
one with the largest terminal dimensions. `latest_client_id` is then updated to
...
2. If preedit changed, the daemon immediately triggers `RenderState.update()` +
   `bulkExport()` outside the normal coalescing window.
3. The daemon applies preedit via `overlayPreedit()` post-`bulkExport()` to
   inject preedit cells into the exported frame data.
...
multi-PANE simultaneous composition; `PreeditState` handles the orthogonal
...
**Non-last pane close, input method switch with `commit_current=false` (§7.5),
and error recovery (§8.5) are the three scenarios where `engine.reset()`
(discard) is used instead of `engine.flush()` (commit).** All other
...
| Error recovery (§8.5)                      | `engine.reset()` (discard)                   | Force return to known-good state after invalid composition state                               |
...
## 11. Silence Detection Timer (P20)

### 11.1 Overview

The daemon provides a per-pane silence detection timer that fires when a pane
produces no PTY output for a client-specified duration. Clients subscribe via
`Subscribe` (0x0810) with `SilenceDetected` and a `silence_threshold_ms`
parameter. When the timer fires, the daemon sends `SilenceDetected` to all
subscribers for that pane.

### 11.2 Timer Lifecycle

The silence timer follows a five-phase lifecycle: arm, reset, fire, re-arm,
disarm.
...
3. **Fire**: When the timer expires (minimum threshold reached), send
   ...
   individual thresholds (minimum-threshold-wins — see §11.5). The timer then
   disarms (`silence_deadline` set to `null`).
...
The timer resets in the PTY read handler, **after `read(pty_fd)`**, **before
...
### 11.4 Per-(Client, Pane) Subscription Tracking

Each pane maintains a bounded subscription list:
...
A global `total_silence_subscribers` counter controls mechanism activation: arm
the mechanism on the 0-to-1 transition, disarm on the 1-to-0 transition.

### 11.5 Minimum-Threshold-Wins Semantics

When multiple clients subscribe to the same pane with different thresholds
(e.g., 10s and 30s), the pane-level timer is set to the minimum (10s). On fire,
`SilenceDetected` is sent to ALL subscribers — no per-client selective firing,
no re-arm at a higher threshold. This is simpler than per-client deadline
tracking: the fire path iterates subscribers and sends to all, then disarms. The
next PTY output re-arms at the minimum threshold again.

Clients requesting higher thresholds receive slightly early notifications —
acceptable for a notification feature.

### 11.6 Cleanup Triggers

Six events cause silence subscriptions to be removed:

| # | Trigger              | Code path                                        | Action                                                 |
| - | -------------------- | ------------------------------------------------ | ------------------------------------------------------ |
| 1 | Explicit Unsubscribe | Message handler                                  | Remove subscription, recalculate min threshold         |
| 2 | Client disconnect    | Client disconnect handler (doc03 Section 3.3)    | Remove all subscriptions for client                    |
| 3 | Connection timeout   | Stale client eviction (Section 3)                | Remove all subscriptions for client                    |
...
| 6 | Pane destruction     | Pane exit cascade (doc03 §3.2), ClosePaneRequest | Cancel all subscriptions for the destroyed pane        |
...
the per-pane timer auto-disarms. When the global `total_silence_subscribers`
count reaches 0, the timer mechanism itself is disabled.

### 11.7 Timer Implementation

Timer implementation is left as implementation discretion. Two viable
approaches:
...
Both produce identical observable behavior. The design specifies the timer
lifecycle semantics (arm/reset/fire/re-arm/disarm, cleanup triggers, per-client
thresholds) without prescribing the kqueue strategy.

### 11.8 Integration

**Pane exit cascade**: Pane destruction (step 3 of the
`executePaneDestroyCascade` procedure in doc03 §3.2) cancels all silence
subscriptions for the destroyed pane. This is cleanup trigger 6 in §11.6. The
cancellation occurs after `PaneMetadataChanged` is sent but before resource
cleanup (PTY close, Terminal deinit).

**Session destroy**: Session destruction iterates all panes and runs per-pane
cleanup, which includes subscription cancellation for each pane.

**Notification subscription**: `SilenceDetected` is an opt-in notification
(Section 9.2). Clients must subscribe via `Subscribe` (0x0810) before receiving
silence notifications.

**Flow control interaction**: The silence timer operates independently of the
flow control system (Section 4). A pane with active silence subscriptions
continues to track PTY output activity regardless of whether attached clients
are paused, stale, or in degraded coalescing tiers.
