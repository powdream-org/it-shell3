...
**Severity**: minor
**Source documents**: `v0.1/design-resolutions/01-daemon-architecture.md` — R1 module description (line 23), R3 decision sentence (line 80)
...
**Severity**: minor
**Source documents**: `v0.2/03-lifecycle-and-connections.md` Section 1.1 Step 6 pseudocode
...
**Severity**: critical
**Source documents**:
- `v0.1/design-resolutions/01-daemon-architecture.md` R1 rationale (line 55): "`pane_slots: [MAX_PANES]?*Pane` **in `server/`**" and "not by pointer, preserving the dependency boundary"
- `v0.2/01-internal-architecture.md` Section 3.3: `pane_slots: [MAX_PANES]?*Pane` inside `Session` annotated as `// core/session.zig`
