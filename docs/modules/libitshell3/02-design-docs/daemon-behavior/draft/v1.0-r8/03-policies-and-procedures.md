...
- **Scope**: Connection limits, multi-client resize, health escalation, flow
  ...
  client state transitions, handshake timeouts, negotiation algorithms
...
| ---------------------- | ----- | ------------------------------------------------------- |
| Minimum concurrent     | 256   | Daemon MUST support at least 256 concurrent connections |
...
- When resource limits are reached, the daemon MUST reject new connections or
  requests with `ERR_RESOURCE_EXHAUSTED` (0x00000600).
- The daemon MAY reject during handshake (fd exhaustion) or on
  `CreateSessionRequest` (session capacity). Both cases use
...
session switching without reconnecting.
...
| READY → AttachSession/CreateSession/AttachOrCreate | 60s      | Send Disconnect(TIMEOUT), close connection      |
