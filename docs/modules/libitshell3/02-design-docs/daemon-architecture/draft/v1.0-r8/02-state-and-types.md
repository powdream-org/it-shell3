...
Type definitions are in `impl-constraints/state-and-types.md` until implemented
in code.
...
`mouse_encode.encode()` + `terminal.flags.mouse_format`. The Surface-level mouse
APIs (`mouseButton()`, `mouseScroll()`, `mousePos()`) are NOT available on
...
sequenceDiagram
    ...
    S->>P: mouse_encode.encode(button, coords,<br/>terminal.flags.mouse_format)
