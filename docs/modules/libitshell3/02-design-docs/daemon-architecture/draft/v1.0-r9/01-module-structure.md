...
Socket setup is delegated to libitshell3-transport (Layer 4).
...
`SessionManager` uses a `[MAX_SESSIONS]?SessionEntry` fixed-size array for
sessions. `MAX_SESSIONS = 64` (ADR 00052). Lookup is linear scan — at most 64
entries, trivially fast. No allocator dependency.

### 1.6 Inter-Library Dependencies

libitshell3, libitshell3-protocol, and libitshell3-transport are separate
libraries with a clean, acyclic dependency relationship (ADR 00060):

```
libitshell3-protocol  (standalone — depends only on Zig std)
libitshell3-transport (standalone — depends only on Zig std; libssh2 added in Phase 5)
...
libitshell3/server/   (depends on core/, ghostty/, input/, libitshell3-ime, libitshell3-protocol, libitshell3-transport)
```
...
- **libitshell3-protocol**: Zig `std` only (codec + framing, Layers 1-2)
- **libitshell3-transport**: Zig `std` only (posix sockets); `libssh2` added in
  Phase 5 for SSH transport
...
| `EVFILT_READ`   | Debug socket listen fd  | Accept debug client connections (ADR 00053, udata=99)      |
...
### 2.6 Category-Based Message Dispatcher

The `server/` module uses a two-level message dispatch pattern (ADR 00064). The
top-level dispatcher routes messages by protocol message type range to
category-specific dispatchers:

| Range  | Category     | Dispatcher                                |
| ------ | ------------ | ----------------------------------------- |
| 0x00xx | Handshake    | `server/dispatch/handshake.zig`           |
| 0x01xx | Session/Pane | `server/dispatch/session_pane.zig`        |
| 0x02xx | Input        | `server/dispatch/input.zig` (stub)        |
| 0x03xx | RenderState  | `server/dispatch/render.zig` (stub)       |
| 0x04xx | Preedit/IME  | `server/dispatch/ime.zig` (stub)          |
| 0x05xx | Subscription | `server/dispatch/subscription.zig` (stub) |

This structure emerged from Plan 7 adding 17 session/pane handlers into a single
switch. Category dispatchers are self-contained — each handles its own message
type range. Plans 8-9 populate the stub dispatchers with input/render/IME
handlers.
