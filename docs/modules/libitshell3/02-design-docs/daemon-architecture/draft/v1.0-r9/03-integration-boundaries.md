...
The daemon communicates with clients through a layered architecture spanning
three modules (ADR 00060):
...
Layers 1-3 are I/O-free (no OS dependencies, no file descriptors, no syscalls).
Layer 4 is the sole layer with OS dependencies.

**External dependencies**: libitshell3-protocol depends on Zig `std` only.
libitshell3-transport depends on Zig `std` (posix sockets, libc); libssh2 added
in Phase 5 for SSH transport. Neither depends on `libitshell3/core/` types —
...
```
┌─────────────────────────────────────────────────────────────────────┐
│ Layer 4: Transport          (libitshell3-transport)                  │
│   SocketConnection, Listener, connect, socket path, stale detection │
│   Has OS dependencies (socket syscalls)                             │
├─────────────────────────────────────────────────────────────────────┤
│ Layer 3: Connection Protocol (libitshell3 server/connection/)        │
│   ConnectionState, 6-state model, sequencing, capability state      │
│   I/O-free state machine                                            │
├─────────────────────────────────────────────────────────────────────┤
│ Layer 2: Framing             (libitshell3-protocol)                  │
│   MessageReader, MessageWriter, fragment reassembly                 │
│   I/O-free, per-connection                                          │
├─────────────────────────────────────────────────────────────────────┤
│ Layer 1: Codec               (libitshell3-protocol)                  │
...
differs (kqueue for daemon, RunLoop/dispatch for Swift client), but the layers
are shared.
...
Handles the 20-byte fixed header (protocol version 2), JSON payloads, and the
hybrid FrameUpdate binary+JSON format. Zero allocation policy — the caller
provides output buffers.
...
- **`MessageReader`** — accumulates bytes fed by the caller, extracts complete
  frames. Uses a tiered buffer strategy (ADR 00061): 64 KB internal fixed buffer
  for common messages, with overflow to a daemon-global `LargeChunkPool` for
  messages exceeding 64 KB. Handles fragment reassembly and incomplete message
  detection.
...
### 1.4 Layer 3 — Connection Protocol (`server/connection/`)

Event-driven state machine, I/O-free. Lives in libitshell3's
`server/connection/` module (not in libitshell3-protocol). Tracks connection
state using the canonical 6-state model from protocol doc 01 Section 5.2:

```
DISCONNECTED -> CONNECTING -> HANDSHAKING -> READY -> OPERATING -> DISCONNECTING
```

Design: `ConnectionState.onEvent(event) -> Action` — deterministic, testable, no
side effects. The daemon or client drives the state machine by feeding events
and executing returned actions.
...
fallback algorithm. Socket files are placed in per-instance directories (ADR
00054):

1. `$ITSHELL3_SOCKET` (explicit override — points to `daemon.sock` file)
2. `$XDG_RUNTIME_DIR/itshell3/<server-id>/daemon.sock`
3. `$TMPDIR/itshell3-<uid>/<server-id>/daemon.sock`
4. `/tmp/itshell3-<uid>/<server-id>/daemon.sock`

Each `<server-id>/` directory contains `daemon.sock`, `debug.sock` (ADR 00053),
and `daemon.pid`. The per-instance directory pattern accommodates multiple
per-instance resources without ad-hoc naming conventions.

**Constraint**: macOS `sockaddr_un.sun_path` is limited to 104 bytes. Socket
paths must stay within this limit.
...
- **`Listener.init(config)`**: Creates per-instance directory before binding.
  `mkdir(<server-id>/, 0700)` -> `socket(AF_UNIX, SOCK_STREAM)` -> stale socket
  detection -> `bind()` -> `listen(backlog)` -> `chmod(0600)` -> `O_NONBLOCK`.
  Returns the listener for kqueue registration via `.fd()`.

- **`Listener.accept()`**: `accept()` + `getpeereid()`/`SO_PEERCRED` UID
  verification + `O_NONBLOCK` + `setsockopt` buffer sizes -> returns a
  `SocketConnection`.

- **`Listener.deinit()`**: `close(listen_fd)` + `unlink(daemon.sock)` +
  `unlink(debug.sock)` + `unlink(daemon.pid)` + `rmdir(<server-id>/)` if empty.
  Compound cleanup ensures no leaked sockets or stale files.

#### 1.5.3 SocketConnection (Both Sides)
...
`transport.connect(config)` (client-side). Named `SocketConnection` (not
`Connection`) to clarify it is a concrete Unix socket implementation, not a
polymorphic abstraction (ADR 00060).
...
pub const SocketConnection = struct {
    ...
    pub fn sendv(self: SocketConnection, iovecs: []posix.iovec_const) SendResult { ... }
    pub fn close(self: *SocketConnection) void { posix.close(self.fd); self.fd = -1; }
};
```
...
- **`.sendv(iovecs)`** — thin wrapper over `posix.writev(self.fd, iovecs)` for
  ring buffer zero-copy path. Returns `SendResult` (same type as `.send()` —
  both wrap `posix.WriteError`).
- **`.close()`** — `posix.close(self.fd)` + `self.fd = -1` (defensive against
  use-after-close)
- **`SocketConnection.fd`** is `pub` — consumer uses it directly for
  kqueue/RunLoop registration
...
1. **SSH swappability (primary)**: In Phase 5, `SocketConnection` will hold SSH
...
2. **Zig idiom**: `std.net.Stream` and `std.fs.File` both own fds and provide
   methods. `SocketConnection` follows the same convention.
...
`connect()` probe to `<server-id>/daemon.sock` + verify PID in `daemon.pid`:

- Success -> daemon already running -> caller decides (daemon exits, client
  connects)
- `ECONNREFUSED` or PID mismatch -> stale socket -> report to caller for cleanup
...
When Phase 5 arrives, the `SocketConnection` struct internals are expanded to
hold SSH channel state, and `.recv()`/`.send()` dispatch to
...
`transport.SocketConnection` (not `TransportListener`/`TransportConnector`). The
types live in the `transport` namespace, so the prefix would be redundant.
...
- **Local**: Client calls `transport.connect(config)` to obtain a
  `SocketConnection` to the daemon's Unix socket directly.
...
- **Layers span three modules**: I/O-free codec + framing (libitshell3-protocol,
  Layers 1-2), connection protocol (libitshell3 server/connection/, Layer 3),
  and transport (libitshell3-transport, Layer 4) — shared by both daemon and
...
All four layers are shared by both daemon and client, distributed across three
modules (ADR 00060):

| Layer               | Module                        | Daemon usage                                            | Client usage                                                 |
| ------------------- | ----------------------------- | ------------------------------------------------------- | ------------------------------------------------------------ |
| **Codec** (L1)      | libitshell3-protocol          | Serialize/deserialize messages                          | Serialize/deserialize messages                               |
| **Framing** (L2)    | libitshell3-protocol          | `MessageReader` + `MessageWriter`                       | `MessageReader` + `MessageWriter`                            |
| **Connection** (L3) | libitshell3 server/connection | `ConnectionState` (server role: sends ServerHello)      | `ConnectionState` (client role: sends ClientHello)           |
| **Transport** (L4)  | libitshell3-transport         | `Listener` + `SocketConnection` (recv/send/sendv/close) | `transport.connect()` + `SocketConnection` (recv/send/close) |

The only difference is event loop integration: the daemon uses kqueue, the
client uses RunLoop/dispatch. The layers provide the building blocks; the
consumer decides when and how to drive I/O.

Socket path resolution, stale socket detection, peer credential extraction, and
socket option configuration are all shared in libitshell3-transport. Without
Layer 4 as a shared module, both daemon and client would independently implement
these multi-step sequences with identical logic and edge cases.
...
| Mouse escape encoding | `ghostty/mouse_encoder.zig` (daemon-authored)    | Encodes mouse events using `terminal.flags.mouse_format`             |
...
sequences directly using daemon-authored `ghostty/mouse_encoder.zig` combined
with `terminal.flags.mouse_format` to determine the correct encoding format
(X10, SGR, etc.).
...
| `mouse_encoder.zig`       | Does NOT exist                                       | Must be written from scratch; reference Surface.zig:3670-3860        |
...
| Helper               | Wraps                                            | Type            | Purpose                                              |
| -------------------- | ------------------------------------------------ | --------------- | ---------------------------------------------------- |
| Terminal lifecycle   | `Terminal.init(alloc, .{.cols, .rows})`          | Wrapper         | Create headless Terminal instance                    |
| VT stream processing | `terminal.vtStream(bytes)`                       | Wrapper         | Feed PTY output into terminal                        |
| RenderState snapshot | `RenderState.update(alloc, &terminal)`           | Wrapper         | Capture terminal state for export                    |
| Cell data export     | `bulkExport(alloc, &render_state, &terminal)`    | Wrapper         | Produce CellData[] for wire transfer                 |
| Key encoding         | `key_encode.encode(writer, event, opts)`         | Wrapper         | Encode key events for PTY (stateless, pure function) |
| Terminal mode query  | `Options.fromTerminal(&terminal)`                | Wrapper         | Read DEC modes, Kitty flags                          |
| Mouse encoding       | `mouse_encoder.zig`                              | Daemon-authored | Encode mouse events for PTY                          |
| Preedit injection    | `overlayPreedit(export_result, preedit, cursor)` | Daemon-authored | Overlay preedit cells post-export                    |
...
## 5. Daemon Memory Resources

### 5.1 LargeChunkPool

The daemon maintains a global `LargeChunkPool` for `MessageReader` overflow (ADR
00061). Most protocol messages fit within `MessageReader`'s 64 KB internal
buffer. For rare large payloads (e.g., bulk clipboard data approaching the 16
MiB framing limit), `MessageReader` borrows a chunk from this pool instead of
allocating from a general-purpose allocator.

| Property           | Value                                                       |
| ------------------ | ----------------------------------------------------------- |
| Chunk size         | 16 MiB (matches maximum protocol payload)                   |
| Initial allocation | 1 chunk in `.bss` (zero runtime allocation for first use)   |
| Growth             | None in v1 — single chunk sufficient for sequential parsing |
| Thread safety      | Not required — single-threaded event loop                   |

The pool is daemon-global, not per-client. Since the event loop processes one
message at a time, a single chunk serves all clients sequentially. The first
chunk lives in `.bss` (statically allocated), so the common case (no large
messages) incurs zero heap allocation.

---

## 6. IME Integration Architecture (libitshell3-ime)

### 6.1 Per-Session ImeEngine Lifecycle
...
### 6.2 Phase 0 -> 1 -> 2 Key Routing Pipeline Overview
...
### 6.3 ImeEngine Vtable and Dependency Direction
...
### 6.5 Critical Runtime Invariant
...
### 6.6 ImeResult to ghostty API Mapping (Phase 2 Detail)
...
### 6.7 Mouse Event Interaction with IME
...
`server/` before encoding the mouse event via `mouse_encoder.encode()`. If
...
sequenceDiagram
    ...
    alt MouseButton (click)
        ...
        S->>P: mouse_encoder.encode() + write(pty_fd)
    else MouseScroll / MouseMove
        S->>P: mouse_encoder.encode() + write(pty_fd)
    end
```

### 6.8 End-to-End Key Input Data Flow
...
### 6.9 Eager Deactivation Rationale

See ADR 00051 (Eager Per-Session IME Deactivation).

---

## 7. Client State Types
...
> **Note**: `AttachSessionRequest` while already in OPERATING state returns
> `ERR_SESSION_ALREADY_ATTACHED`. The client must explicitly
> `DetachSessionRequest` first, then `AttachSessionRequest` to switch sessions
> (ADR 00020). This is not a state transition and is therefore not shown in the
> diagram.

### 7.2 ClientState Struct
...
**Ownership chain**: `ClientState` -> `ConnectionState` (Layer 3, sequence
tracking + capabilities) -> `SocketConnection` (Layer 4, fd + I/O methods).

| Field              | Description                                                                                                                                                     |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `client_id`        | Monotonically increasing identifier assigned at `accept()`                                                                                                      |
| `connection_state` | Layer 3 connection protocol state machine (6-state model, sequence tracking, capability negotiation)                                                            |
...
| `message_reader`   | Per-connection framing state. Accumulates partial messages across `recv()` calls. Requires `LargeChunkPool` reference for messages exceeding 64 KB (ADR 00061). |

---

## 8. Remote Version Conflict Handling
...
The daemon always interacts with `transport.SocketConnection` values. Whether a
client connected locally or through an SSH tunnel is invisible to the daemon:
...
flowchart LR
    ...
    D1 -.- Note1["Daemon sees:<br/>transport.SocketConnection<br/>from Listener.accept()"]
    D2 -.- Note2["Daemon sees:<br/>transport.SocketConnection<br/>from Listener.accept()"]
```

The daemon has no "local vs remote" code path. All clients are
`SocketConnection` values with `recv()`, `send()`, `sendv()`, and `close()`.
This is a structural property of the architecture, not an abstraction to be
maintained.

---

## 10. Items Deferred to Future Versions

| Item                                                 | Deferred to | Rationale                                                                                               |
| ---------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------- |
| SSH transport implementation                         | Phase 5     | No remote use case in v1. SocketConnection struct designed for transparent swap.                        |
| Transport vtable/interface                           | Phase 5     | One implementation (Unix socket) in v1. Abstract when the second arrives.                               |
| `SocketConnection.deinit()` with SSH channel cleanup | Phase 5     | v1 SocketConnection holds only an fd (4-byte struct). SSH adds channel state requiring compound deinit. |
