...
| I/P-frame model with periodic keyframes          | **Decided**         | Eliminates per-client dirty bitmaps (O(N) -> O(1)). Provides auto-healing state drift every keyframe interval (1s default). Two `frame_type` values: 0 = P-frame (partial), 1 = I-frame (full keyframe). `frame_type=2` (I-unchanged) removed in v1.0-r12 — idle panes produce no frames instead of byte-identical duplicates.                          |
...
- **doc 01**: Updated Section 11.3 design decisions table — `frame_type` entry
  corrected from three values to two (removed `frame_type=2` I-unchanged per
  R6). Removed stale `ghostty_surface_preedit()` references (lines 364, 386,
  399).

### v0.11 (2026-03-10)

- **Cross-team revision with daemon v0.3**: Extracted daemon behavioral
  descriptions from all 6 protocol docs. Protocol docs now focus on wire format,
  message definitions, and error codes. Daemon-internal behavior (process
  lifecycle, IME engine lifecycle, health escalation, coalescing policies, ring
  buffer architecture, resize algorithms) is defined in daemon design docs.
- **doc 01**: Removed auto-start procedure (P1), FD passing section (P2),
  heartbeat initiation policy (P19), health escalation timeline (P7), connection
  limit number (P3), resize algorithm internals (P4), coalescing
  tiers/timing/WAN adaptation (P10), auth syscalls/permissions (P5).
- **doc 02**: Removed preedit ownership algorithm (P11), multi-client resize
  algorithm and hysteresis (P4), reconnection step-by-step procedure (P6), auth
  implementation details (P5).
- **doc 03**: Removed SIGHUP/PTY cleanup details from pane close (P14), preedit
  flush-to-PTY from focus change (P12), TIOCSWINSZ/debounce from resize (P14),
  stale exclusion/hysteresis details, IME engine lifecycle from input method
  state (P17).
- **doc 04**: Removed PTY independence guarantee from frame suppression (P15),
  coalescing details (P10).
- **doc 05**: Removed preedit ownership model/algorithm/sequence diagram (P11),
  PTY commit details from screen switch and focus change (P12), coalescing tier
  table/thresholds/latency targets (P10).
- **doc 06**: Removed ring buffer architecture/sizing/implementation (P7/P8/P9),
  health escalation timeline/stale triggers/smooth degradation, coalescing tier
  definitions/WAN adjustments/power throttling, IME engine reconstruction from
  session restore (P17). Added daemon-side note to notification defaults (P18).

### v0.8 (2026-03-07)

- **Preedit overhaul alignment** (V2-01): Updated doc 01 to reflect the v0.8
  preedit-is-cell-data architecture per design resolution
  `01-preedit-overhaul.md`. Preedit rendering is through cell data in
  I/P-frames; the server injects preedit cells into the terminal grid and they
  appear in FrameUpdate CellData. The client renders all cells uniformly.
- **Removed preedit from JSON metadata blob**: Removed "preedit" from all
  FrameUpdate JSON metadata descriptions (Sections 1.2, 3.2 encoding table,
  4.2.1 encoding convention, 4.2 hybrid encoding table, Section 9.1 bandwidth
  estimate). The JSON metadata blob contains cursor, colors, and dimensions
  only.
- **Replaced dual-channel design**: Replaced the "preedit dual-channel design"
  normative block (Section 4.2) with the v0.8 single-path model: dedicated
  preedit messages (0x0400-0x0405) are lifecycle/metadata only; rendering is via
  cell data.
- **Updated PreeditUpdate description**: PreeditUpdate (0x0401) description
  changed from "pushes current composition state" to lifecycle/metadata for
  multi-client coordination.
- **Removed preedit bypass**: Removed "preedit bypass" paragraph from Section
  5.8 (I/P-frame model) and "Preedit bypasses everything" from Section 10.3
  (coalescing properties). All frames go through the ring buffer; Coalescing
  Tier 0 ensures immediate flush.
- **Updated health table**: Section 5.7 stale client frame delivery changed from
  "None (except preedit bypass)" to "None (ring cursor stagnant)".
- **Updated coalescing Preedit tier**: Section 10.1 Preedit tier description
  changed from "Bypasses all coalescing" to "Server writes frame to ring
  immediately".
- **Fixed frame_type claim**: Section 11.3 design decisions table updated to
  reflect v0.8 renumbering: 3 values {0=P-partial, 1=I-frame, 2=I-unchanged}.
  Removed stale backward-compatibility claim about frame_type values 1/2
  matching old dirty values.

### v0.7 (2026-03-05)

- **ClientHealthChanged added to registry** (Issue 1): Added
  `ClientHealthChanged` (0x0185, S->C) to the Session & Pane Management message
  type table between `ClientDetached` (0x0184) and `WindowResize` (0x0190).
- **Resize policy and health model overview** (Issue 2): Added Sections 5.6
  (Multi-Client Resize Policy), 5.7 (Client Health Model), and 5.8 (I/P-Frame
  Model and Shared Ring Buffer) providing architectural overview of new v0.7
  features with cross-references to docs 03, 04, and 06.
- **I/P-frame model in design goals**: Updated Section 1.1 "Efficient rendering
  updates" to describe I/P-frame model and shared ring buffer. Updated Section
  1.2 principles 2 (server-authoritative, per-session IME) and 7 (backpressure
  with ring buffer and I-frame seek).
- **Per-session IME**: Updated Section 5.5 multi-session model to reflect
  per-session (not per-pane) IME engine architecture and preedit exclusivity
  invariant.
- **Comparison tables updated**: Added multi-client output and resize policy
  rows to tmux (Section 8.1) and zellij (Section 8.2) comparison tables. Updated
  iTerm2 comparison (Section 8.3) for I-frame recovery.
- **I-frame bandwidth analysis**: Added Section 9.3 with I-frame bandwidth
  overhead estimates per pane.
- **Coalescing model updated**: Updated Section 10.3 preedit bypass (ring buffer
  bypass path) and smooth degradation (ring cursor lag replaces queue filling).
- **Design decisions table**: Added 7 new decided entries: I/P-frame model,
  shared ring buffer, cumulative P-frame diff base, `latest` resize default,
  two-state health model, per-session IME engine.

### v0.6 (2026-03-05)

- **Exhaustive message type registry** (Issue 1): Added all missing pane
  response types (0x0141, 0x0143, 0x0145, 0x0147, 0x0149, 0x014B, 0x014D,
  0x014F, 0x0151, 0x0153), WindowResizeAck (0x0191), and per-message entries for
  doc 06 ranges (Flow Control 0x0500-0x0506, Clipboard 0x0600-0x0604,
  Persistence 0x0700-0x0707, Notifications 0x0800-0x0813, Extensions
  0x0A00-0x0A02). Doc 01 registry is now the single exhaustive index of all
  protocol message types.
- **ERR_PROTOCOL_ERROR defined** (Issue 2): Added `ERR_PROTOCOL_ERROR`
  (0x00000007) to Section 6.3 error code table. Used for generic protocol
  violations such as setting reserved flags.
- **Heartbeat direction clarified** (Issue 14): Updated Section 5.4 prose to
  explicitly state heartbeat is bidirectional (matching the message type table),
  with the typical case being server-initiated.
