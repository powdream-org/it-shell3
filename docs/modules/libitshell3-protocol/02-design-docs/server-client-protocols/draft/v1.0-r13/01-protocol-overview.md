...
5. **Event-driven delta delivery, not fixed-fps.** The server does not render at
   ...
   an adaptive cadence model (see Section 9). The coalescing ceiling matches the
   client's display refresh rate (typically 16ms for 60 Hz) but real terminal
   workloads typically produce 0-30 updates/second.
...
| Path             | `$XDG_RUNTIME_DIR/itshell3/<server-id>/daemon.sock` or `$TMPDIR/itshell3-<uid>/<server-id>/daemon.sock` |
...
flowchart TD
    ...
    C -->|Yes| D["$XDG_RUNTIME_DIR/itshell3/&lt;server-id&gt;/daemon.sock"]
    C -->|No| E{"$TMPDIR set?"}
    E -->|Yes| F["$TMPDIR/itshell3-&lt;uid&gt;/&lt;server-id&gt;/daemon.sock"]
    E -->|No| G["/tmp/itshell3-&lt;uid&gt;/&lt;server-id&gt;/daemon.sock"]
```

The `<server-id>` is a short identifier (default: `default`) allowing multiple
daemon instances. Each instance uses a per-instance directory containing
`daemon.sock`, `debug.sock` (debug subsystem), and `daemon.pid` (PID file). The
`$ITSHELL3_SOCKET` override points to the `daemon.sock` file specifically, not
the instance directory. The `debug.sock` and `daemon.pid` files are not part of
the client protocol.
...
### 3.1 Frame Header (20 bytes, fixed)

Every message on the wire begins with this 20-byte header:

```
Offset  Size  Field          Description
------  ----  -----          -----------
 0      2     magic          Magic bytes: 0x49 0x54 (ASCII "IT")
 2      1     version        Header format version (current: 2; see Section 3.1.1)
...
12      8     sequence       Sequence number (little-endian u64)
```

**Total header size: 20 bytes** (natural 8-byte alignment for sequence field)

**Important**: `payload_len` is the size of the payload only. Total bytes on the
wire = 20 + payload_len. The 2-byte reserved field provides alignment padding
only — no semantic meaning or future-use documentation assigned.

#### 3.1.1 Version Byte Semantics

The version byte identifies the **binary header format**, not the protocol
feature set. Currently `2`. Exact match is required in the reader loop (Section
9.2).

**Evolution policy:** All backward-compatible protocol evolution uses capability
negotiation (doc 02). The version byte is incremented only when the 20-byte
...
```
+----------------------+----------------------------------+
|  Header (20 bytes)   |  Payload (payload_len bytes)     |
...
```
+----------------------+---------------------+--------------------+-------------------+
|  Header (20 bytes)   |  Binary frame hdr   |  Binary DirtyRows  |  JSON metadata    |
...
1. Read exactly 20 bytes (header)
...
The sequence counter is u64. Wrap-around is not a practical concern (u64 at 1000
messages/second lasts over 584 million years).
...
```
[20-byte binary header (ENCODING=1)] -> dispatch on type + encoding flag
    |
...
stateDiagram-v2
    ...
    READY --> OPERATING : SessionAttach / SessionCreate
...
| `READY`         | `AttachSessionRequest` / `CreateSessionRequest` success | `OPERATING`     | Begin session interaction             |
...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| **New tab**    | Open new Unix socket connection -> ClientHello/ServerHello -> CreateSessionRequest (or AttachSessionRequest with `create_if_missing`) |
...
The server enforces a compile-time maximum of `MAX_CLIENTS = 64` simultaneous
connections. When a new client attempts to connect and the limit is reached, the
server rejects the connection with `ERR_RESOURCE_EXHAUSTED` during the handshake
phase (before completing capability negotiation). The specific lifecycle stage
at which this occurs is defined in daemon design docs.
...
| `ref_sequence` | u64    | Sequence number of the message that caused the error (0 if unsolicited) |
...
### 8.1 FrameUpdate Size Estimates
...
### 8.3 I-Frame (Keyframe) Bandwidth Overhead
...
### 10.1 Unix Socket Authentication
...
### 10.2 SSH Tunnel Authentication
