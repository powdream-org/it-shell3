...
sequenceDiagram
    ...
    else Attach or create (create_if_missing)
        Client->>Server: AttachSessionRequest (0x0104)<br/>{create_if_missing: true, session_name: "..."}
        Server->>Client: AttachSessionResponse (0x0105)<br/>{action_taken: "attached" | "created"}
    end
...
```
[20-byte header: magic=IT, version=2, flags=0x00 (ENCODING=0, JSON), msg_type=0x0001, payload_len=<N>, seq=1]
...
### 8.3 Attach-or-Create Semantics

`AttachSessionRequest` (0x0104) supports attach-or-create semantics via the
`create_if_missing` field, equivalent to tmux's `new-session -A`. When
`create_if_missing` is true and the named session does not exist, the server
creates it and attaches. The response includes `action_taken` (`"attached"` or
`"created"`) to distinguish the outcome. See doc 03 Section 1.5 for field
definitions.
...
daemon. See doc 01 Section 10.2 for details.
