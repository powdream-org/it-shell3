...
All messages use the binary framing defined in document 01 (20-byte header:
magic(2) + version(1) + flags(1) + msg_type(2) + reserved(2) + payload_len(4) +
sequence(8), little-endian byte order).
...
The binary framing header (20 bytes) wraps a JSON object as the payload body.
...
- Ratios are fixed-point u32 integers (x10^4, range 0-10000, where 5000 =
  50.00%). JSON representation uses integer values (e.g., `"ratio": 5000`).
...
**Field length constraints**: `name` max 64 bytes UTF-8. The server MUST reject
requests exceeding this limit with `ERR_FIELD_TOO_LONG`.
...
| Field               | Type    | Description                                                                                                                                                                          |
| ------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `session_id`        | u32     | Session to attach to (used when attaching by ID)                                                                                                                                     |
| `session_name`      | string  | Session name to attach to (used when attaching by name). Max 64 bytes UTF-8. Empty string = attach to most recently active session, or create if `create_if_missing` and none exist. |
| `create_if_missing` | boolean | If true and the named session does not exist, create it then attach. Equivalent to tmux `new-session -A`. Default: false.                                                            |
...
The client provides either `session_id` or `session_name` (not both). If
`session_id` is non-zero, it takes precedence. If `session_name` is provided and
`create_if_missing` is true, the server creates the session if it does not
exist.
...
| `action_taken`           | string | `"attached"` or `"created"` (present when `create_if_missing` was true in the request)                     |
| `session_id`             | u32    | Session ID                                                                                                 |
| `pane_id`                | u32    | Initial pane ID (only meaningful when `action_taken` = `"created"`)                                        |
...
The `ratio` is a fixed-point u32 (x10^4, range 0-10000). It describes the
proportion of space given to the **first** child (the child containing the
original pane in a right/down split, or the new pane in a left/up split).
...
Adjusts the split divider adjacent to a pane. The `orientation` indicates the
split axis to resize, and `delta_ratio` is a signed fixed-point ratio delta
(x10^4).
...
| Field         | Type | Description                                                                           |
| ------------- | ---- | ------------------------------------------------------------------------------------- |
| `session_id`  | u32  | Session containing the pane                                                           |
| `pane_id`     | u32  | Pane adjacent to the split divider                                                    |
| `orientation` | u8   | Split axis: 0 = horizontal, 1 = vertical                                              |
| `delta_ratio` | i32  | Signed fixed-point ratio delta (x10^4). +500 = grow by 5.00%. -500 = shrink by 5.00%. |

**Semantics**: The server finds the nearest ancestor split node matching
`orientation` and applies `new_ratio = old_ratio + delta_ratio`, clamping to
`[MIN_RATIO, 10000 - MIN_RATIO]` where `MIN_RATIO = 500` (5%) to prevent
zero-size panes. The client computes the ratio delta directly; the daemon
...
| Field    | Type   | Description                                                                                 |
| -------- | ------ | ------------------------------------------------------------------------------------------- |
| `status` | number | 0 = success, 1 = pane not found, 2 = no split in that orientation, 3 = minimum size reached |

### 2.14 EqualizeSplitsRequest (0x014C)

Sets all split ratios in a session's layout tree to 5000 (equal distribution).
...
The `ratio` field is a fixed-point u32 (x10^4, range 0-10000, where 5000 =
50.00%). JSON representation uses integer values. `MIN_RATIO = 500` (5%)
prevents zero-size panes.
...
The server enforces a maximum tree depth of 4 levels (`MAX_TREE_DEPTH = 4`),
derived from the 16-pane limit (`MAX_PANES = 16`) in a binary split tree.
Clients must be prepared to handle trees up to this depth.
...
**Field length constraints**: `title` max 256 bytes, `cwd` max 4096 bytes. These
fields originate from OSC sequences (shell output), not client input. The server
MUST truncate at a UTF-8 character boundary if the source data exceeds the limit
(TRUNCATE policy for OSC-originated fields).
...
| 9    | `FIELD_TOO_LONG`      | A string field exceeds its maximum byte length       |
...
| Session/pane mutation | CreateSessionRequest, DestroySessionRequest, RenameSessionRequest, AttachSessionRequest (with `create_if_missing: true`)                                                  |
