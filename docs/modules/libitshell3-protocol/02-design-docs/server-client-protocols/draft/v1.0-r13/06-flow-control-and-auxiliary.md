...
[doc 01 Section 9](01-protocol-overview.md#9-frameupdate-delivery-model) for the
wire-observable delivery model.
...
> **Normative**: Clipboard payload data MUST NOT exceed 10 MiB. The receiver
> MUST reject payloads exceeding this limit with an error response
> (`payload_too_large`). This is a message-specific constraint within the 16 MiB
> maximum payload size defined in the frame header.
...
1. Passes through the base64 string with `encoding: "base64"`.
2. Sends a ClipboardWrite to all attached clients with the base64 data.
