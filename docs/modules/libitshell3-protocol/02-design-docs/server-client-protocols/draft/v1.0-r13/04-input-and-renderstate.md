...
All messages share the 20-byte binary header defined in
[Doc 01 §3.1](./01-protocol-overview.md#31-frame-header-20-bytes-fixed).
...
| -------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `keycode`      | u16    | HID usage code, e.g., `0x04` = 'A'. The daemon maps this to `hid_keycode: u8` (bounded by HID_KEYCODE_MAX = 0xE7) when passing to the IME engine. |
...
> **Normative note — CapsLock/NumLock semantic bits**: CapsLock (bit 4) and
> NumLock (bit 5) carry semantic information required by native IME engines for
> character case resolution and numpad key classification. These bits MUST NOT
> be treated as optional or ignorable by receivers.
...
```
[20-byte binary header] [binary frame header] [binary DirtyRows + CellData] [JSON metadata blob]
```
...
------  ----  -----                -----------
 0      20    [header]             type=0x0300, payload_len=variable
20       4    session_id           Session identifier (u32 LE)
24       4    pane_id              Pane identifier (u32 LE)
28       8    frame_sequence       Monotonic frame counter (u64 LE)
36       1    frame_type           Frame type (see below)
37       1    screen               0=primary, 1=alternate
38       2    section_flags        Bitflags indicating which sections follow (u16 LE)
```
...
---     -----               -----------
 ...
 1      rle_encoded          Row uses run-length encoding (existing, unchanged; see Section 4)
...
rows)**: 20 (header) + 20 (frame header) + 4 (json_len) + ~60 (JSON) = **~104
...
---------------------------------
Total:          ~32,774 bytes (worst case, all unique styles)
```
...
target. See doc 01 Section 9 for the wire-observable delivery model.
...
All input messages use JSON payloads (20-byte binary header + JSON body).
...
000C    XX XX XX XX XX XX XX XX                   sequence (u64)

        -- Binary frame header (20 bytes) --
0014    01 00 00 00                               session_id = 1
0018    01 00 00 00                               pane_id = 1
001C    2A 00 00 00 00 00 00 00                   frame_sequence = 42
0024    00                                        frame_type = 0 (P-frame, partial)
0025    00                                        screen = primary
0026    90 00                                     section_flags = 0x0090 (DirtyRows + JSONMetadata)

        -- DirtyRows Section (binary) --
0028    01 00                                     num_dirty_rows = 1

        -- Row 0 header --
002A    0A 00                                     y = 10
002C    00                                        row_flags = 0x00 (no selection, no RLE,
                                                    semantic_prompt=none, no hyperlink)
002D    04 00                                     selection_start (no selection active)
002F    04 00                                     selection_end (no selection active)
0031    05 00                                     num_cells = 5

        -- Cell 0: 'H' (16 bytes) --
0033    48 00 00 00                               codepoint = 0x48 ('H')
0037    00                                        wide = narrow
0038    01 00                                     flags = bold
003A    00                                        content_tag = 0 (codepoint)
...
0047    00                                        wide = narrow
0048    01 00                                     flags = bold
004A    00                                        content_tag = 0 (codepoint)
004B    00 00 00 00                               fg = default
004F    00 00 00 00                               bg = default

        ... (cells 2-4 follow same pattern) ...

        -- GraphemeTable (empty) --
0083    00 00                                     num_entries = 0

        -- UnderlineColorTable (empty) --
0085    00 00                                     num_entries = 0

        -- JSON Metadata Blob --
0087    YY YY YY YY                               json_len = N
008B    7B 22 63 75 72 73 6F 72 ...               {"cursor":{"x":5,"y":10,"visible":true,
