...
3. A full I-frame (`frame_type=1 or frame_type=2`) for each visible pane from the shared ring buffer. **Note**: Because a freshly attaching client has no prior I-frame, it MUST treat `frame_type=2` as `frame_type=1`. See doc 04 Section 7.3 ("Client processing (seeking)") for the normative rule.
...
4. Processes the I-frame (`frame_type=1 or frame_type=2`) to render the initial viewport — treating it as `frame_type=1` per doc 04 Section 7.3 (initial attach is a seeking context)
...
This is immediately followed by an I-frame (`frame_type=1 or frame_type=2`) for the initial pane. **Note**: The newly created session has no prior I-frame, so the client MUST treat `frame_type=2` as `frame_type=1`. See doc 04 Section 7.3 ("Client processing (seeking)").
