...
This document specifies the wire protocol for managing the Session > Pane hierarchy in libitshell3. Each Session owns a single binary split tree of Panes. There is no intermediate "Tab" concept — the host application (e.g., ghostty) provides tab management in its own UI, mapping each libitshell3 Session to one host tab.
