...
1. **Hybrid encoding: binary framing + binary CellData + JSON control.** Every message has a fixed 16-byte binary header for O(1) dispatch. The header's encoding flag (bit 0 of flags byte) indicates the payload format: `0` = JSON payload, `1` = binary payload. `FrameUpdate` uses binary encoding for DirtyRows/CellData (the bulk of the data) with a trailing JSON metadata blob for cursor, colors, and dimensions. All other messages — handshake, session management, input events, errors — use JSON payloads for debuggability, schema evolution, and cross-language ease (Swift `JSONDecoder`, `socat | jq`).

2. **Server-authoritative state.** The daemon owns terminal state (PTY, scrollback, preedit, cursor). Clients are thin renderers that receive state updates and forward raw input. This simplifies multi-client consistency. The server owns the native IME engine (libitshell3-ime) — one engine instance per session, shared by all panes in the session. Clients never perform composition.
...
7. **Backpressure-aware.** Flow control messages and a shared per-pane ring buffer prevent buffer bloat when clients cannot keep up with rendering updates. Slow clients are advanced to the latest I-frame (keyframe) in the ring rather than accumulating stale deltas.
...
| ID | Name | Direction | Encoding | Description |
|----|------|-----------|----------|-------------|
| `0x0300` | `FrameUpdate` | S->C | **Hybrid** (binary + JSON) | RenderState delta or full update. Binary DirtyRows/CellData (including preedit cells injected via `ghostty_surface_preedit()`) + JSON metadata blob. |
...
See doc 05 for detailed message specifications. The server owns the native IME engine (libitshell3-ime). Clients send raw HID keycodes via KeyEvent; the server manages composition lifecycle and pushes preedit metadata to all attached clients. Preedit rendering is through cell data in I/P-frames (the server calls `ghostty_surface_preedit()` and preedit cells appear in FrameUpdate CellData). All messages in this range use JSON encoding.
...
**Preedit architecture (v0.8)**: Preedit is cell data, not metadata. The server calls `ghostty_surface_preedit()`, which injects preedit cells into the terminal grid. These cells are serialized as part of FrameUpdate CellData in I/P-frames. The client renders all cells uniformly — it has no concept of which cells are preedit. Dedicated preedit messages (0x0400-0x0405) provide lifecycle/metadata only (multi-client coordination, debugging); they are not used for rendering. The `"preedit"` capability controls only these dedicated messages; preedit rendering via cell data is always available regardless of capability negotiation.
...
- **Independent IME state**: IME engine instances are per-session (not per-connection or per-pane). Each session has one IME engine shared by all panes in the session. Creating a new connection to a new session creates a new IME engine instance, providing natural preedit isolation between sessions. Per-session libhangul instances are trivially cheap (~few KB each). When multiple connections attach to the same session, they share the same per-session IME state (with per-session locking for preedit ownership). At most one pane in a session can have active preedit at any time (preedit exclusivity invariant).
...
The server uses an I-frame/P-frame model with periodic keyframes for multi-client rendering state delivery, analogous to video codec keyframes:
...
Frames are stored in a **shared per-pane ring buffer** (default 2 MB). The server serializes each frame once into the ring. Per-client read cursors track delivery position. This provides O(1) frame serialization and O(1) memory per frame regardless of client count.

**Recovery**: All recovery scenarios (ContinuePane after PausePane, buffer overrun, stale recovery) collapse into a single operation: advance the client's ring cursor to the latest I-frame. The I-frame IS the full state resync — same data, same wire format, no special codepath.

**Preedit delivery**: All frames, including those containing preedit cell data, go through the ring buffer. There are no bypass paths. Coalescing Tier 0 (Preedit tier) ensures immediate flush on preedit state change, maintaining <33ms preedit latency over Unix socket.

See doc 04 Section 4 for the `frame_type` wire format and I/P-frame semantics. See doc 06 Section 2 for ring buffer sizing, socket write priority, and recovery procedures.
...
| I/P-frame model with periodic keyframes | **Decided** | Eliminates per-client dirty bitmaps (O(N) -> O(1)). Provides auto-healing state drift every keyframe interval (1s default). Three `frame_type` values: 0 = P-partial, 1 = I-frame, 2 = I-unchanged (renumbered in v0.8 after preedit bypass removal). |
| Shared per-pane ring buffer | **Decided** | Server serializes each frame once into ring. Per-client read cursors (12 bytes each) replace per-client buffers (512KB each). Memory: O(panes x ring_size) + O(clients) for cursors. 100 clients, 4 panes: 8 MB ring + cursors vs 200 MB per-client buffers. |
| Cumulative P-frame diff base (Option B) | **Decided** | P-frames reference most recent I-frame (not previous frame). Any P-frame independently decodable with just the current I-frame. No sequential chain. Clients may skip intermediate P-frames freely. Enables shared ring buffer without per-client state tracking. |
| `latest` resize policy as default | **Decided** | Matches tmux 3.1+ default. Active device always gets full screen real estate. `smallest` available as opt-in server config. Stale clients excluded from resize calculation to prevent idle device constraining active device. |
| Two-state client health model | **Decided** | `healthy` and `stale` only. `paused` is orthogonal flow-control. Smooth degradation is server-internal (not protocol-visible). Minimizes protocol surface area while covering practical scenarios. |
| Per-session IME engine | **Decided** | One IME engine per session (not per-pane). All panes in session share engine. Flush (commit) on intra-session pane focus change. At most one pane per session can have active preedit (preedit exclusivity invariant). |
...
- **Preedit overhaul alignment** (V2-01): Updated doc 01 to reflect the v0.8 preedit-is-cell-data architecture per design resolution `01-preedit-overhaul.md`. Preedit rendering is through cell data in I/P-frames; the server calls `ghostty_surface_preedit()` and preedit cells appear in FrameUpdate CellData. The client renders all cells uniformly.
- **Removed preedit from JSON metadata blob**: Removed "preedit" from all FrameUpdate JSON metadata descriptions (Sections 1.2, 3.2 encoding table, 4.2.1 encoding convention, 4.2 hybrid encoding table, Section 9.1 bandwidth estimate). The JSON metadata blob contains cursor, colors, and dimensions only.
- **Replaced dual-channel design**: Replaced the "preedit dual-channel design" normative block (Section 4.2) with the v0.8 single-path model: dedicated preedit messages (0x0400-0x0405) are lifecycle/metadata only; rendering is via cell data.
- **Updated PreeditUpdate description**: PreeditUpdate (0x0401) description changed from "pushes current composition state" to lifecycle/metadata for multi-client coordination.
- **Removed preedit bypass**: Removed "preedit bypass" paragraph from Section 5.8 (I/P-frame model) and "Preedit bypasses everything" from Section 10.3 (coalescing properties). All frames go through the ring buffer; Coalescing Tier 0 ensures immediate flush.
- **Updated health table**: Section 5.7 stale client frame delivery changed from "None (except preedit bypass)" to "None (ring cursor stagnant)".
- **Updated coalescing Preedit tier**: Section 10.1 Preedit tier description changed from "Bypasses all coalescing" to "Server writes frame to ring immediately".
- **Fixed frame_type claim**: Section 11.3 design decisions table updated to reflect v0.8 renumbering: 3 values {0=P-partial, 1=I-frame, 2=I-unchanged}. Removed stale backward-compatibility claim about frame_type values 1/2 matching old dirty values.

### v0.7 (2026-03-05)

- **ClientHealthChanged added to registry** (Issue 1): Added `ClientHealthChanged` (0x0185, S->C) to the Session & Pane Management message type table between `ClientDetached` (0x0184) and `WindowResize` (0x0190).
- **Resize policy and health model overview** (Issue 2): Added Sections 5.6 (Multi-Client Resize Policy), 5.7 (Client Health Model), and 5.8 (I/P-Frame Model and Shared Ring Buffer) providing architectural overview of new v0.7 features with cross-references to docs 03, 04, and 06.
- **I/P-frame model in design goals**: Updated Section 1.1 "Efficient rendering updates" to describe I/P-frame model and shared ring buffer. Updated Section 1.2 principles 2 (server-authoritative, per-session IME) and 7 (backpressure with ring buffer and I-frame seek).
- **Per-session IME**: Updated Section 5.5 multi-session model to reflect per-session (not per-pane) IME engine architecture and preedit exclusivity invariant.
- **Comparison tables updated**: Added multi-client output and resize policy rows to tmux (Section 8.1) and zellij (Section 8.2) comparison tables. Updated iTerm2 comparison (Section 8.3) for I-frame recovery.
- **I-frame bandwidth analysis**: Added Section 9.3 with I-frame bandwidth overhead estimates per pane.
- **Coalescing model updated**: Updated Section 10.3 preedit bypass (ring buffer bypass path) and smooth degradation (ring cursor lag replaces queue filling).
- **Design decisions table**: Added 7 new decided entries: I/P-frame model, shared ring buffer, cumulative P-frame diff base, `latest` resize default, two-state health model, per-session IME engine.
