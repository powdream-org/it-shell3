...
The per-pane `frame_sequence` counter is incremented for every frame written to the ring buffer. All rendering frames (I-frames, P-frames) go through the ring — there are no bypass paths for rendered content. The sole exception is scroll-response I-frames, which are delivered via the per-client direct message queue because scroll is a per-client viewport operation (see doc 04 Section 6.1).
...
- **Preedit bypass buffer removed** (Resolution 7): Removed the per-client preedit bypass buffer (Section 2.2). All frame data, including preedit cell data, is delivered through the shared ring buffer. Preedit latency is maintained by Coalescing Tier 0 (immediate flush). Socket write priority simplified from 3 levels to 2 levels: direct message queue + ring buffer frames. Resolution 17 from v0.7 is superseded.
- **Ring contents updated** (Resolution 7, 8): Ring now contains ALL frames. `frame_sequence` incremented for every frame — no bypass exception. `frame_type` renumbered from 4 values {0=P-metadata, 1=P-partial, 2=I-frame, 3=I-unchanged} to 3 values {0=P-partial, 1=I-frame, 2=I-unchanged}. Resolutions 18-20 from v0.7 are superseded.
- **PausePane preedit delivery note removed** (Resolution 7): Removed preedit-specific bypass delivery during PausePane and the preedit-only FrameUpdate format blockquote. Paused clients receive preedit cell data via normal ring cursor advancement.
- **Stale health state updated**: Frame delivery column changed from "preedit bypass only" to "None (ring cursor stagnant)" — no separate bypass path exists.
- **Smooth degradation updated**: Removed "Continue delivering preedit-only frames via bypass buffer" from the 90% ring lag step.
...
- **~~Preedit bypass buffer~~ (Resolution 17)**: ~~Preedit-only frames (`frame_type=0` with preedit state change) delivered via per-client latest-wins priority buffer outside the ring.~~ **Superseded by v0.8 preedit overhaul** — all frames now go through the ring buffer. Preedit latency maintained by Coalescing Tier 0.
- **~~Ring contents and frame_sequence scope~~ (Resolutions 18-20)**: ~~Ring contains I-frames, P-frames, and non-preedit metadata frames. `frame_sequence` incremented only for ring frames.~~ **Superseded by v0.8 preedit overhaul** — ring contains ALL frames, `frame_sequence` incremented for all frames.
...
- **~~Preedit-only FrameUpdate for paused clients~~ (Issue 9/Gap 12)**: ~~Defined preedit bypass FrameUpdate format.~~ **Superseded by v0.8 preedit overhaul** — preedit is cell data delivered through the ring, no separate bypass format.
...
- **~~Preedit exempt from PausePane~~ (Section 2.1)**: ~~Preedit-only frames (~90 bytes) MUST be delivered even when a pane is paused.~~ **Superseded by v0.8 preedit overhaul** — preedit is delivered through the ring like all other frame data.
