...
**Server response**: A FrameUpdate with `frame_type=1` (I-frame) showing the scrolled viewport. This I-frame is delivered via the per-client direct message queue (priority 1), NOT the shared ring buffer, because scroll is a per-client viewport operation — writing it to the shared ring would expose a scrolled viewport to all clients, including those that did not request the scroll.
