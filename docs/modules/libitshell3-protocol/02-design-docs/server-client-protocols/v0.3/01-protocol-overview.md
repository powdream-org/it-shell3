...
**Changes from v0.2**: Hybrid encoding (binary CellData + JSON control), reframed fps language to event-driven delta delivery with coalescing ceiling, added encoding flag to header, added semantic CellData design principle, updated bandwidth notes
...
---  ----            -----------
 ...
 1   COMPRESSED      Payload is zstd-compressed (see Section 3.5)
...
When the `COMPRESSED` flag is set, the payload is zstd-compressed. Compression is applied after encoding (binary or JSON) and before transmission. Compression is optional and negotiated during handshake.
...
| Render State: **FrameUpdate** | **Hybrid** (binary CellData + JSON metadata) | **1** | Binary for bulk cell data (3x smaller, RLE-compatible); JSON for cursor, preedit, colors, dimensions (debuggable, `"han"` not hex) |
| Render State: other (Scroll, Search) | JSON | 0 | Low frequency |
| CJK & IME | JSON | 0 | Preedit shows `"han"` not hex; low frequency |
...
| `0x0003` | `Heartbeat` | Bidirectional | JSON | Keepalive ping (carries timestamp + ping_id) |
| `0x0004` | `HeartbeatAck` | Bidirectional | JSON | Keepalive pong (echoes timestamp + ping_id, adds responder timestamp) |
| `0x0005` | `Disconnect` | Bidirectional | JSON | Graceful disconnect with reason |
| `0x0006` | `Error` | Bidirectional | JSON | Structured error report |
...
| Cursor, Preedit, Colors, Dimensions | JSON blob | Debuggable; preedit shows `"han"` not hex; low-frequency metadata |
...
|-------|------|-------------|
| `timestamp` | u64 | Milliseconds since Unix epoch (sender's clock) |
...
```json
{
  "timestamp": 1709568000123,
  "ping_id": 42,
  "responder_timestamp": 1709568000125
}
```

| Field | Type | Description |
|-------|------|-------------|
| `timestamp` | u64 | Echoed from Heartbeat (sender's original timestamp) |
| `ping_id` | u32 | Echoed from Heartbeat |
| `responder_timestamp` | u64 | Responder's current time (for RTT and clock skew estimation) |

**RTT calculation**: `RTT = current_time - ack.timestamp`. One-way latency is approximately `RTT / 2`.
...
The `Error` message (`0x0006`) uses JSON encoding (ENCODING=0):
...
| **Optional fields** | JSON: field absent or `null`. Binary: preceding u8 flag (`0x00`/`0x01`). | JSON payloads use native JSON optionality. Binary payloads use explicit presence flags. |
| **Timestamps** | u64, milliseconds since Unix epoch | Used for heartbeat timing and debugging. In JSON payloads, encoded as a number. |
...
| **Input forwarding** | `send-keys` text command via control mode | JSON `KeyEvent` message with HID keycode, modifiers, layout ID | Lower latency, richer key info (modifier disambiguation, layout awareness) |
...
- **Client hints**: `ClientDisplayInfo` provides `display_refresh_hz`, `power_state`, `preferred_max_fps` for server-side adaptation
- **iOS power**: Auto-reduce fps when client reports battery (cap Active@20fps, Bulk@10fps)
- **Preedit latency requirement**: <33ms end-to-end over Unix socket (MUST)
...
pub const FrameHeader = extern struct {
    ...
    pub const FrameFlags = packed struct(u8) {
        encoding: bool = false,     // 0=JSON, 1=binary
