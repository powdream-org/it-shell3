...
**Changes from v0.2**:
- **All CJK/IME messages use JSON payloads**: Converted all preedit lifecycle messages (0x0400-0x0403), input method messages (0x0404-0x0406), and IMEError (0x04FF) from binary offset tables to JSON encoding. Consistent with doc 01's encoding convention (ENCODING=0 for all non-FrameUpdate messages). Composition state uses string enum values instead of u8.
- **Preedit section is JSON-encoded**: Preedit data is now part of the FrameUpdate JSON metadata blob (doc 04 Section 4.2). Updated all references and examples.
- **Preedit is highest-priority coalescing tier**: MUST flush immediately on preedit state change, regardless of current tier.
- **Preedit bypasses PausePane**: Preedit-only frames (~90 bytes) are exempt from flow control. Changed from SHOULD to MUST.
- **Preedit bypasses power throttling**: Always immediate regardless of `power_state`.
- **Preedit latency target**: <33ms end-to-end over Unix socket (normative MUST).
- **Per-pane preedit cadence**: A pane with active composition runs at Preedit tier even if adjacent panes are in Bulk.
...
| `client_id` | u32 | Client that initiated composition |
| `cursor_x` | u16 | Composition start column |
| `cursor_y` | u16 | Composition start row |
| `active_layout_id` | u16 | Keyboard layout (same IDs as KeyEvent) |
...
**Reason values**:
- `"committed"`: Normal completion (Space, Enter, non-Jamo key)
- `"cancelled"`: User pressed Escape during composition
...
| `active_layout_id` | u16 | Keyboard layout |
| `display_width` | u8 | Width of preedit text in cells |
...
| `"leading_jamo"` | Single consonant: ㅎ, ㄴ, ㄱ, ... |
| `"syllable_no_tail"` | Consonant + vowel: 하, 나, 가, ... |
| `"syllable_with_tail"` | Full syllable (C+V+C): 한, 난, 간, ... |
| `"double_tail"` | Syllable with double final consonant: 읽 (ㅇ+ㅣ+ㄹㄱ) |
...
Note: Backspace during `leading_jamo` produces a PreeditEnd with `reason="cancelled"` and empty committed text, because the composition was fully undone without committing anything.
...
|-------|------|-------------|
| `pane_id` | u32 | Target pane |
| `active_layout_id` | u16 | The now-active layout |
| `previous_layout_id` | u16 | The previously active layout |
...
4. **Preedit latency target**: The end-to-end latency from KeyEvent receipt to FrameUpdate delivery MUST be less than **33ms** over a Unix domain socket. This is a normative requirement. The 33ms budget covers: KeyEvent read (~0ms) + IME processing (~0.1ms) + preedit state update (~0.1ms) + FrameUpdate serialization (~0.1ms) + socket write (~0ms) = ~0.3ms typical, with 32.7ms margin for scheduling jitter.
...
**Cursor style during composition**: The server automatically adjusts `cursor.style` in the FrameUpdate JSON metadata during composition (see doc 04, Section 4.2). Specifically:
- `cursor.style` is set to block (0) and `cursor.blinking` to false during composition
- `cursor.x` and `cursor.y` equal `preedit.cursor_x` and `preedit.cursor_y` — the block cursor visually encloses the composing character
...
| `0x0404` | InputMethodSwitch | C -> S | JSON | Request keyboard layout change |
| `0x0405` | InputMethodAck | S -> C | JSON | Confirm layout change |
