...
- **ghostty Cell is 64 bits packed**: codepoint (21 bits), style_id (indirect), wide marker (2 bits), flags. Styles are reference-counted in a per-page `RefCountedSet`. Wide chars use two cells (wide + spacer_tail). (`page.zig:1958-2002`)
- **ghostty style indirection is the export bottleneck**: Each `page.Cell` stores a `style_id` — an opaque index into a per-page `RefCountedSet`. To export cell data, the server must dereference this index to resolve colors and flags. This is the dominant cost in `bulkExport()` (~11 ns/cell). The client side avoids this cost entirely — `importFlatCells()` builds `RenderState.Cell` directly from the pre-resolved flat data (~4 ns/cell). (`style.zig:20-200`, `render_export.zig`)
...
- **SSH compression is complementary to our binary format**: SSH's `zlib@openssh.com` uses persistent zlib context (dictionary builds up across packets). Our 16-byte FlatCell layout is highly compressible — empty cells are mostly zeros, consecutive cells share colors. Estimated 10-20x for typical terminals, 3-5x for dense output. Client-side `Compression yes` required (off by default). No application-layer compression needed in v1.
