...
**Changes from v0.2**:
- **Hybrid encoding for FrameUpdate**: Binary section (frame header + DirtyRows + CellData) followed by JSON metadata blob (cursor, preedit, colors, dimensions, mouse, terminal modes). Added `section_flags` bit 7 to indicate JSON metadata presence.
- **CellData is SEMANTIC**: Added normative note clarifying CellData carries semantic content, not GPU-aligned data. The client performs font shaping, glyph atlas lookup, and GPU buffer construction.
- **Reframed Section 8.3**: Replaced "Target: 60 fps maximum" with event-driven coalescing model. 16ms minimum interval (coalescing ceiling). FrameUpdates sent only when dirty state exists.
- **Updated bandwidth table (Section 8.1)**: Added "coalesced ceiling, not sustained target" note to Heavy output row.
- **Cursor blink**: Added normative note — client runs local blink timer; server does NOT send FrameUpdates for blink animation.
- **Input messages**: Explicitly stated that all input messages (KeyEvent, MouseButton, etc.) use JSON payloads.
...
------  ----  -----        -----------
 ...
 3      1     flags        Per-message flags (bit 0 = ENCODING: 0=JSON/1=binary,
                                       bit 1 = COMPRESSED, bit 2 = RESPONSE,
...
| `active_layout_id` | u16 | Keyboard layout identifier |
...
------                                    ------
 ...
 [User presses key]
     |
     v
 KeyEvent(JSON: HID keycode, mods, layout)
     |
...
The JSON blob contains whichever metadata sections are relevant for this frame. All fields are optional — only changed or required fields are included:
...
**Cursor behavior during CJK composition**: When `preedit.active` is true (see Preedit below), the server overrides `cursor.style` to block (0) and `cursor.blinking` to false for the duration of composition. The pre-composition cursor style is restored when composition ends. During composition, `cursor.x` and `cursor.y` MUST equal `preedit.cursor_x` and `preedit.cursor_y` — the block cursor visually encloses the composing character. When composition commits, `cursor.x` advances past the committed character to the normal insertion point.
...
When `active` is false, the client clears its preedit overlay. The `text` field is absent or empty.
...
| `cursor_color` | [r, g, b] or null | Cursor RGB (null = no cursor color) |
| `palette` | [[r,g,b], ...] or null | Full 256-entry palette (null = unchanged) |
| `palette_changes` | [[index, [r,g,b]], ...] or null | Partial palette updates |
...
When compression is enabled (header flags bit 1 COMPRESSED = 1):
...
**Note**: The JSON metadata blob is compressed together with the binary sections as part of the overall payload. There is no separate compression for JSON vs binary.
...
|------|------|-------------|
| `0x0200` | KeyEvent | Raw HID keycode + modifiers (JSON) |
