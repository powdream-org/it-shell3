...
**Changes from v0.6**: I/P-frame model, shared ring buffer, multi-client resize policy, client health model, per-session IME (see Changelog)
...
**Preedit bypass**: Preedit-only frames bypass the ring entirely and are delivered directly per-client via a latest-wins priority buffer, ensuring <33ms preedit latency regardless of ring cursor position.
...
|-----------|------|-------|
| Binary frame header | 8 B | Pane ID, frame_type, screen, row/col counts |
...
- **Preedit bypasses everything**: Coalescing, PausePane, ring buffer, power throttling. Preedit-only frames are delivered via a per-client latest-wins priority buffer outside the ring (~110B/frame = negligible cost).
...
| I/P-frame model with periodic keyframes | **Decided** | Eliminates per-client dirty bitmaps (O(N) -> O(1)). Provides auto-healing state drift every keyframe interval (1s default). No capability negotiation needed — backward wire-compatible (`frame_type` values 1/2 match old `dirty` values 1/2). |
