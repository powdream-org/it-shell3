...
### 1.7 Idle Coalescing Suppression During Resize Debounce
...
After the 500ms grace period expires, normal coalescing tier transitions resume.

**Interaction with resize debounce (doc 03 Section 5.1)**: The 250ms resize debounce window begins when the server computes a new effective size for a pane and arms the debounce timer. The 500ms grace period begins when `ioctl(TIOCSWINSZ)` fires (i.e., when the debounce timer expires). The total window during which Idle transitions are suppressed is at most 750ms per resize event.
...
| `resize_exclusion_timeout_ms` | number (u32) | Grace period in ms before a paused client is excluded from the resize calculation (0 = server default; default 5000). Transport-independent. |
| `stale_timeout_ms` | number (u32) | PausePane duration in ms before the client is declared `stale` and peers are notified via ClientHealthChanged (0 = server default; default 60000 for local, 120000 for ssh_tunnel). |
| `eviction_timeout_ms` | number (u32) | Total PausePane duration in ms before the server sends Disconnect with reason `"stale_client"` (0 = server default; default 300000). Transport-independent. |

> **Transport-aware stale timeout**: The server selects the stale_timeout_ms default based on `ClientDisplayInfo.transport_type`. Local (Unix socket) uses 60000 ms; SSH tunnel uses 120000 ms; unknown defaults to 60000 ms. The client MAY override by supplying a non-zero value. The 5s resize-exclusion grace period and 300s eviction timeout remain transport-independent.
...
```json
{
  ...
  "effective_resize_exclusion_timeout_ms": 5000,
  "effective_stale_timeout_ms": 60000,
  "effective_eviction_timeout_ms": 300000
}
```
...
| `effective_resize_exclusion_timeout_ms` | number (u32) | Actual resize exclusion timeout in effect for this client |
| `effective_stale_timeout_ms` | number (u32) | Actual stale timeout in effect for this client |
| `effective_eviction_timeout_ms` | number (u32) | Actual eviction timeout in effect for this client |
...
3. Queue filling above 90% capacity:
   ...
   → Continue delivering preedit-only FrameUpdates (num_dirty_rows=0 + preedit metadata).
...
### 2.6 PausePane Health Escalation Timeline

When a client enters the PausePane state and does not send ContinuePane, the server escalates through a series of timed actions. All timeout values use the effective values negotiated via FlowControlConfig (Section 2.3).

```
T=0s:    PausePane sent. Client is still `healthy`. Still participates in
         the resize calculation.

T=5s:    Resize exclusion (default resize_exclusion_timeout_ms = 5000).
         Server recalculates the effective terminal size without this client.
         No protocol message is sent. This is a server-internal decision.
...
(local): Server sends ClientHealthChanged (0x0185) to all peer clients
         attached to the same session. The affected client is NOT notified
         (it is still paused; it already knows).

T=120s   `stale` health state transition (SSH tunnel transport).
(SSH):   Same behavior as T=60s. The longer timeout accounts for higher
         latency and more variable behavior over SSH tunnels.

T=300s:  Eviction (default eviction_timeout_ms = 300000).
         If active preedit composition exists for the evicted client,
         the server commits it (flushes to the terminal grid) and sends
         PreeditEnd with reason "client_evicted" to all peer clients
         before the Disconnect (see Section 2.7).
         Server sends Disconnect with reason "stale_client" and tears down
         the connection. Transport-independent.
```

**Stale timeout reset rules**: The stale timeout clock resets ONLY when the client sends a message that proves application-level processing:
...
**Output queue stagnation as stale trigger**: In addition to PausePane duration, the server uses output queue stagnation as a stale trigger:
...
**Transport-aware timeout selection**: The server selects stale_timeout_ms defaults based on `ClientDisplayInfo.transport_type`:
...
### 2.7 Preedit Commit on Client Eviction

When the server evicts a stale client at T=300s (the eviction_timeout_ms threshold), any active preedit composition owned by that client MUST be committed (flushed to the terminal grid) before the client connection is torn down. This prevents orphaned composition state — if the user was mid-composition when the client became stale, the partial composition is committed to the terminal so it is not lost.

The server sends `PreeditEnd` with `reason: "client_evicted"` to all remaining peer clients before the `Disconnect`. See doc 05 Section 2.3 for the PreeditEnd reason value definition.

**Sequence at eviction**:

```
1. If active preedit composition exists for the evicted client's pane:
   a. Commit the composition (flush jamo buffer to terminal grid).
   b. Send PreeditEnd { reason: "client_evicted" } to all peer clients.
2. Send Disconnect { reason: "stale_client" } to the evicted client.
3. Tear down the connection.
```

Peer clients receive PreeditEnd before Disconnect, ensuring they remove the preedit overlay before the session state stabilizes. The evicted client receives only Disconnect (it is still connected at tear-down time but is paused; it does not receive the PreeditEnd).
...
5. For each restored pane, the server re-initializes the IME engine (see below).
...
9. Server sends FrameUpdate for each pane.

**IME engine initialization**: When restoring a session, the server MUST re-initialize the IME engine for each restored pane:

1. Create a new `ImeEngine` instance with the saved `input_method` string from the session snapshot (e.g., `"korean_2set"`)
...
The session snapshot must persist per-pane: `input_method` (canonical string, e.g., `"direct"`, `"korean_2set"`). See IME Interface Contract, Section 3.7 for the canonical registry of valid `input_method` strings.
...
- ClientHealthChanged (0x0185, doc 03, always sent) — no subscription required, matching the 0x0180-0x018x convention
...
The `0x0900-0x09FF` range is reserved for future connection health extensions. The first allocation in this range will be `echo_nonce` (v2, HEARTBEAT_NONCE capability, message types `0x0900`/`0x0901`). See "Heartbeat Orthogonality" note below.
...
### Heartbeat Orthogonality Note

Heartbeat (0x0003-0x0005) is a **connection liveness** mechanism. Its 90-second timeout triggers `Disconnect(TIMEOUT)`. This is independent from the client health state model (Section 2.6).

The two systems serve different purposes and must not be conflated:

| System | Trigger | Action |
|--------|---------|--------|
| Heartbeat (connection liveness) | No message received for 90s | `Disconnect(TIMEOUT)` — connection is dead |
| Health state (application responsiveness) | PausePane duration or output queue stagnation | `stale` state + ClientHealthChanged — connection is alive but app is unresponsive |

Possible combinations:

| Combination | Meaning |
|-------------|---------|
| Heartbeat-healthy + app stale | `stale` (app frozen, TCP alive) — standard escalation path |
| Heartbeat-missed + app healthy | Connection problem (resolves or disconnects at 90s timeout) |

**`echo_nonce` deferred to v2**: Application-level heartbeat verification (to distinguish "app frozen" from "genuinely idle PTY") is deferred to v2 as the `HEARTBEAT_NONCE` capability, using message types in the `0x0900` reserved range. For v1, the combination of `latest` resize policy + output queue stagnation detection + PausePane escalation timeline covers practical scenarios. The idle-PTY blind spot (no output = no queue growth = no detection of frozen client) is mitigated by `latest` policy: an idle client's dimensions are irrelevant when another client is active.
...
| PausePane without ContinuePane | 5s / 60s (local) or 120s (SSH) / 300s | Escalating: resize exclusion at 5s, `stale` + ClientHealthChanged at 60s/120s, Disconnect `"stale_client"` at 300s (see Section 2.6) |
...
**Applied from design-resolutions-resize-health.md and review-notes/02-cross-document-consistency.md (Issues 12, 14 partial, 16, 17, 18, 19, 20). Issues 13, 15, and the discard-and-resync portion of Issue 14 are deferred pending the I-frame/P-frame design discussion (Phase 3, Issues 22-24).**

- **Issue 12 (CRITICAL): FlowControlConfig new timeout fields**: Added `resize_exclusion_timeout_ms` (u32, default 5000), `stale_timeout_ms` (u32, default 60000 local / 120000 SSH), and `eviction_timeout_ms` (u32, default 300000) to FlowControlConfig (0x0502) payload. Added corresponding `effective_resize_exclusion_timeout_ms`, `effective_stale_timeout_ms`, and `effective_eviction_timeout_ms` fields to FlowControlConfigAck (0x0503). Added transport-aware stale timeout note: server selects stale_timeout_ms default from ClientDisplayInfo.transport_type (Section 2.3).

- **Issue 14 partial (CRITICAL): PausePane health escalation timeline**: Added new Section 2.6 "PausePane Health Escalation Timeline" documenting the 5s/60s(local)/120s(SSH)/300s escalation sequence, stale timeout reset rules (application-level messages only, not HeartbeatAck), output queue stagnation as stale trigger (>90% for stale_timeout_ms with no app-level messages), and transport-aware timeout selection table. The discard-and-resync portion of Issue 14 (Resolution 14/15) is deferred (see below).

- **Issue 16 (HIGH): Section 10 timeout table updated**: Replaced "PausePane without ContinuePane: No timeout, Server compacts queue, waits indefinitely" with the 5s/60s-120s/300s escalation summary referencing Section 2.6.

- **Issue 17 (HIGH): Heartbeat orthogonality note**: Added "Heartbeat Orthogonality Note" subsection in Section 7 clarifying that heartbeat (connection liveness, 90s → Disconnect) and health states (application responsiveness, PausePane duration / output queue) are independent systems with a comparison table. Added note that `echo_nonce` (application-level heartbeat verification) is deferred to v2 as `HEARTBEAT_NONCE` capability using message types in the `0x0900` reserved range. Updated the `0x0900` range description in Section 7 opening paragraph.

- **Issue 18 (MEDIUM): Idle coalescing suppression during resize debounce**: Added new Section 1.7 "Idle Coalescing Suppression During Resize Debounce" specifying that the server MUST NOT transition a pane's coalescing tier to Idle during the 250ms resize debounce window and for 500ms after `ioctl(TIOCSWINSZ)` fires. Total suppression window is at most 750ms per resize event.

- **Issue 19 (MEDIUM): Preedit commit on client eviction**: Added new Section 2.7 "Preedit Commit on Client Eviction" specifying the mandatory commit-before-eviction sequence: commit active composition, send PreeditEnd `"client_evicted"` to peer clients, then Disconnect `"stale_client"` to the evicted client.

- **Issue 20 (MEDIUM): ClientHealthChanged in default subscriptions**: Added `ClientHealthChanged (0x0185, doc 03, always sent)` to the Default Subscriptions list in Section 6, matching the always-sent convention for 0x0180-0x018x notifications.

**Deferred (Issues 13, 15, and partial Issue 14)**:
- **Issue 13**: Per-(client, pane) buffer limit reduction from 1 MB to 512 KB — deferred pending the I-frame/P-frame design discussion (Phase 3, Issues 22-24). The buffer limit in the Server Output Queue Management table remains 1 MB.
- **Issue 15**: Discard-and-resync procedure and stale recovery resync sequence — deferred pending I-frame/P-frame design. These may become "skip to keyframe" in the new model.
- **Issue 14 (discard-and-resync part)**: The buffer-overflow discard path referenced in the escalation timeline is deferred for the same reason.
