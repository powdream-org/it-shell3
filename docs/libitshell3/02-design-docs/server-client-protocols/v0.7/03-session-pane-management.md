...
2. A full `FrameUpdate` (I-frame) for each visible pane.
...
On success, the same post-attach sequence applies as for AttachSessionResponse: LayoutChanged, FrameUpdate (I-frame), PreeditSync (if applicable), ClientAttached notification.
...
On WindowResize or KeyEvent from client C:
  ...
  6. During the debounce window:
     ...
     - Preedit bypass frames are still sent.
