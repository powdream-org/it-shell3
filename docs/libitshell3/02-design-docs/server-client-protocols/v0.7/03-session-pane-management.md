...
| `resize_policy` | string | Server's active resize policy: `"latest"` or `"smallest"`. Informational — not negotiated. See Section 5.1. |
...
> **Always-sent vs opt-in notifications**: The notifications in this section (0x0180-0x0185: LayoutChanged, PaneMetadataChanged, SessionListChanged, ClientAttached, ClientDetached, ClientHealthChanged) are **always sent** to all clients attached to the affected session -- no subscription is required. In contrast, doc 06 Section 5 defines opt-in notifications (0x0800-0x0806: PaneTitleChanged, ProcessExited, Bell, RendererHealth, PaneCwdChanged, ActivityDetected, SilenceDetected) that require explicit subscription via the Subscribe/Unsubscribe mechanism (doc 06 Section 6).
...
Sent to all peer clients attached to the same session when a client's health state changes. NOT sent to the affected client itself (it already knows — it received PausePane or is processing its recovery).
...
| Field | Type | Description |
|-------|------|-------------|
| `session_id` | u32 | Session the affected client is attached to |
| `client_id` | u32 | The affected client |
| `client_name` | string | Human-readable client name (from ClientHello) |
| `health` | string | New health state: `"healthy"` or `"stale"` |
| `previous_health` | string | Previous health state |
| `reason` | string | Reason for transition (see table below) |
| `excluded_from_resize` | boolean | Whether the client is now excluded from the resize calculation |

**`reason` values**:

| Value | Description |
|-------|-------------|
| `"pause_timeout"` | PausePane duration exceeded the stale timeout (60s local / 120s SSH) |
| `"queue_stagnation"` | Output queue >90% for the stale timeout with no application-level messages sent |
| `"recovered"` | Client sent ContinuePane or resumed processing; health returned to `"healthy"` |
...
**Health state model**: The protocol defines two health states:

| State | Definition | Resize participation | Frame delivery |
|-------|-----------|---------------------|----------------|
| `healthy` | Normal operation | Yes | Full (per coalescing tier) |
| `stale` | PausePane too long or output queue stagnant | No (excluded after 5s grace) | None (except preedit bypass frames) |

A client in PausePane (flow control) remains `healthy` until the stale timeout fires. Smooth degradation (server-internal auto-tier-downgrade) is NOT a protocol-visible health state and does NOT trigger ClientHealthChanged. See doc 06 Section 2 for the full PausePane escalation timeline (5s resize exclusion → 60s/120s stale → 300s eviction) and stale trigger details.
...
The server supports two resize policies, configurable at the session level (server configuration; reported to clients via `AttachSessionResponse.resize_policy` but not negotiated):

| Policy | Effective size | Use case |
|--------|---------------|----------|
| `latest` (default) | Dimensions of the most recently active client | Single user, multiple devices (macOS + iPad). Active device always gets full screen real estate. |
| `smallest` (opt-in) | `min(cols)` × `min(rows)` across all eligible clients | Multi-user shared sessions where every participant must see unclipped content. |

**Client eligibility**: Clients in the `stale` health state (see Section 4.6) are excluded from the resize calculation under both policies. All other clients participate. Excluding only stale (not transiently-slow) clients prevents resize flapping from transient backpressure bursts.

**`latest` policy — `latest_client_id` tracking**: The server tracks `latest_client_id` per session, updated when a client sends a KeyEvent or WindowResize message. HeartbeatAck does NOT update `latest_client_id` (passive liveness does not indicate active use). When the latest client detaches or becomes stale, the server falls back to the next most-recently-active healthy client. If no activity record exists for any client, the server falls back to the client with the largest terminal dimensions.

**`smallest` policy — stale exclusion**: Stale clients are removed from the min() calculation. If no healthy clients remain attached, the server retains the last known effective dimensions.

**Resize debounce**: `ioctl(fd, TIOCSWINSZ)` is debounced at 250ms per pane, preventing SIGWINCH storms during rapid resize drags. Behavior:

1. When a resize target is computed for a pane, arm a 250ms timer.
2. If another resize arrives within the 250ms window, reset the timer and update the target dimensions.
3. Fire `ioctl(TIOCSWINSZ)` only when the timer expires.
4. **Exception**: The first resize after session creation or client attach fires immediately (no debounce).
5. During the debounce window, suppress FrameUpdate generation for the intermediate dimensions (only preedit bypass frames are sent).
6. During the debounce window and for 500ms after `ioctl(TIOCSWINSZ)` fires, the server MUST NOT transition the pane's coalescing tier to Idle (see doc 06).

**Re-inclusion hysteresis**: When a stale client recovers to `healthy`, it must remain healthy for 5 seconds before being re-included in the resize calculation. During this window the client receives frames normally but does not affect PTY dimensions. After 5 seconds of sustained healthy state the server re-includes the client and triggers a resize cascade if the effective size changes. This prevents resize churn from rapid stale/healthy oscillations.

**Algorithm (both policies)**:

```
on WindowResize or KeyEvent from client C:
  update C.last_activity_time
  if policy == latest and session.latest_client_id != C.id:
...
recompute_effective_size(session):
  eligible = session.attached_clients
    .filter(c => c.health == healthy)
    .filter(c => policy == latest
                 ? c.id == session.latest_client_id
                 : true)
  if eligible is empty:
    retain last known effective dimensions
    return
  if policy == latest:
    effective = eligible[0].dimensions  // latest client's dims
  else:  // smallest
    effective = (min(c.cols for c in eligible),
                 min(c.rows for c in eligible))
  if effective != session.effective_size:
    arm 250ms debounce timer per affected pane

on detach of client C:
  remove C from eligible set
  recompute_effective_size(session)

on timer expiry (per pane):
  ioctl(pane.pty_fd, TIOCSWINSZ, effective_size)
  walk layout tree, recompute pane dimensions from split ratios
  send LayoutChanged to ALL attached clients
  send FrameUpdate for each pane whose content changed
```

**Viewport clipping under `latest` policy**: When the effective terminal size exceeds a client's reported dimensions (WindowResize cols/rows), the client MUST render only the top-left region corresponding to its own dimensions. Content beyond the client's viewport boundary is clipped. The protocol does not provide per-client viewport scrolling in v1. Per-client viewports (scroll to see clipped areas) are deferred to v2.
...
Notifications (LayoutChanged, PaneMetadataChanged, etc.) use the server's next monotonic sequence number. Notifications are identifiable by their message type — notification types (0x0180-0x0185) are distinct from request/response types. Sequence number 0 is never sent on the wire; it is used only as a sentinel value in payload fields (e.g., `ref_sequence = 0` in Error messages means "no specific message triggered this error").
...
The effective terminal size for a session is determined by the server's active resize policy (reported in `AttachSessionResponse.resize_policy`). Under the default `latest` policy, PTY dimensions are set to the most recently active client's size (activity determined by KeyEvent or WindowResize, not HeartbeatAck). Under the opt-in `smallest` policy, the effective size is `min(cols)` x `min(rows)` across all eligible healthy clients. Stale clients are excluded from both calculations. See Section 5.1 for the full resize algorithm, debounce behavior, and re-inclusion hysteresis.
...
- When a session is restored from a snapshot, the server re-initializes the IME engine for each pane using the saved `input_method` string. Composition state is not restored. See doc 06 Section 4.4 (RestoreSessionResponse) for the full IME initialization sequence.
...
- **ClientHealthChanged (0x0185)** (Issue 4 / Resolution 12): New always-sent notification added as Section 4.6. JSON payload carries `session_id`, `client_id`, `client_name`, `health` ("healthy"/"stale"), `previous_health`, `reason` ("pause_timeout"/"queue_stagnation"/"recovered"), and `excluded_from_resize`. Sent to peer clients; not sent to the affected client itself. Includes normative health state model table. Added to message type assignments table (Issue 8).
- **Section 4 introductory note updated** (Issue 8): Always-sent range updated from 0x0180-0x0184 to 0x0180-0x0185 to include ClientHealthChanged.
- **AttachSessionResponse `resize_policy` field** (Issue 6 / Resolution 4): Added `resize_policy` field (`"latest"` or `"smallest"`) to Section 1.6. Informational — reports the server's active policy; not negotiated by the client.
- **Section 5.1 resize algorithm rewritten** (Issue 5 / Resolutions 1, 2, 3, 5, 6): Replaced the single `min(cols) × min(rows)` description with the full dual-policy model: `latest` (default, PTY dimensions = most recently active client's size) and `smallest` (opt-in, traditional min). Documents `latest_client_id` tracking (updated on KeyEvent/WindowResize, not HeartbeatAck), stale-client exclusion from both policies, 250ms debounce per pane with first-attach exception, Idle coalescing suppression during debounce + 500ms grace, and 5-second re-inclusion hysteresis after stale recovery.
- **Viewport clipping normative statement** (Issue 21): Added normative statement at end of Section 5.1: under `latest` policy, clients with smaller dimensions than the effective terminal size MUST clip to the top-left region. Per-client viewport scrolling deferred to v2.
- **Section 7 notification range updated**: Notification types note updated from 0x0180-0x0184 to 0x0180-0x0185.
- **Section 8 Window Size updated** (Issue 7): Replaced `min(cols) × min(rows)` description with reference to the dual-policy model in Section 5.1. Notes `latest` as default, `smallest` as opt-in, stale exclusion, and `AttachSessionResponse.resize_policy`.
- **Section 9 Readonly Receives updated**: Added `ClientHealthChanged` to the list of messages received by readonly clients.

### v0.6 (2026-03-05)

- **Always-sent vs opt-in notification distinction** (Issue 16): Added normative note in Section 4 clarifying that notifications 0x0180-0x0184 are always sent (no subscription required), while doc 06 Section 5 notifications (0x0800-0x0806) require explicit subscription.

### v0.5 (2026-03-05)

- **RestoreSession IME initialization** (cross-review): Added cross-reference in Section 8 (Input Method State) noting that session restore re-initializes the IME engine per-pane. Full IME initialization sequence added to doc 06 Section 4.4 (RestoreSessionResponse).
- **Input method identifier unification** (identifier consensus): Updated Section 8 session restore reference to use single canonical `input_method` string instead of `layout_id` + `active_language` pair.
