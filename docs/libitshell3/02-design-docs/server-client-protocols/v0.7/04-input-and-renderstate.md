...
**Changes from v0.6**:
- No substantive changes. Version bump for consistency with doc 05. Issues 23-24 (I-frame/P-frame) deferred to Phase 3.

**Changes from v0.5** (consistency review):
- **Issue 3: ERR_DECOMPRESSION_FAILED removed**: Replaced with `ERR_PROTOCOL_ERROR` in Section 9 (compression reserved flag violation).
- **Issue 12/13: Readonly cross-references**: Updated Section 2.8 to reference doc 03 Section 9 as authoritative for readonly permissions.
- **Issue 19: display_width in FrameUpdate preedit**: Added `display_width` (u8) field to FrameUpdate preedit JSON section (Section 4.2), making FrameUpdate self-contained for preedit overlay rendering. Eliminates client-side UAX #11 dependency.
- **Issue 20: Coalescing terminology**: Clarified "4-tier" refers to 4 active coalescing tiers plus Idle quiescent state.

**Changes from v0.4** (cross-review: Protocol v0.4 x IME Interface Contract v0.3):
- **Issue 1: IME routing validation**: Added normative note that server MUST validate `keycode <= 0xE7` before routing to IME engine (Section 2.1).
- **Issue 2: Wire-to-IME KeyEvent mapping**: Added new subsection documenting how the server decomposes wire modifier bitmask into IME contract's separated fields (Section 2.1).
...
The `keyboard_layout` axis (e.g., `"qwerty"`, `"dvorak"`) is a separate, orthogonal per-pane property and is NOT encoded in the `input_method` string. It is established at handshake and omitted from KeyEvent. In v1, only `"qwerty"` is supported.
