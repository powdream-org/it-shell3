...
**Changes from v0.6**: Added `"stale_client"` Disconnect reason; updated resize algorithm and AttachSessionResponse for dual-policy model (see Changelog)
...
| `resize_policy` | string | Server's active resize policy: `"latest"` or `"smallest"`. Informational — not negotiated. See doc 03 Section 5.1. |
...
1. **Terminal dimensions**: Computed according to the server's resize policy (`latest` or `smallest`). `latest` (default) uses the most recently active client's dimensions; `smallest` uses min(cols, rows) across all eligible clients. See doc 03 Section 5.1 for the full algorithm.
...
The server supports two resize policies (`latest` default, `smallest` opt-in) with stale client exclusion, 250ms resize debounce per pane, and 5-second re-inclusion hysteresis after stale recovery. See doc 03 Section 5.1 for the full normative algorithm.

Under `latest` policy, clients with smaller dimensions than the effective size MUST clip to their own viewport (top-left origin). Per-client virtual viewports (scroll to see clipped areas) are deferred to v2.
...
- **`"stale_client"` Disconnect reason added** (Issue 3, Resolution 8): Added `"stale_client"` to the Disconnect reason enum in Section 11.1. This reason is sent by the server when evicting a client that has remained `stale` for the eviction timeout (default 300s). See doc 06 Section 2 for the full eviction timeline.
- **Resize algorithm updated for dual-policy model** (Verification round 2): Section 9.6 and 9.9 updated to reference the `latest`/`smallest` dual resize policy (doc 03 Section 5.1) instead of hardcoded `min()`. Section 9.9 algorithm replaced with summary and cross-reference to eliminate duplication.
- **`resize_policy` added to AttachSessionResponse** (Verification round 2): Added `resize_policy` field to Section 9.2 JSON example and field table, matching doc 03 Section 1.6.
