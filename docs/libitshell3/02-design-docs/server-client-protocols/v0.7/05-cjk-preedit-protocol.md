...
**Changes from v0.6**:
- **Issue 10 (cross-doc consistency): Add `"client_evicted"` PreeditEnd reason**: Added `"client_evicted"` to Section 2.3 PreeditEnd reason values. Sent when a stale client is evicted at T=300s and the server commits any active preedit before the Disconnect.
- **Issue 01 (residual empty-state): Replace `empty + vowel` with `null + vowel`**: Fixed two explanatory prose notes in Sections 3.2 and 3.3 that still used the obsolete `empty` label instead of `null`.
...
- `"client_evicted"`: The composing client was evicted by the server (stale client eviction at T=300s). The server commits any active preedit text to the PTY and sends PreeditEnd with this reason to remaining peer clients before sending Disconnect to the evicted client.
...
**Wire format**: The FrameUpdate has `num_dirty_rows=0` (section_flags bit 4 NOT set) with only the JSON metadata blob (section_flags bit 7 set) containing the preedit section. No grid cell data is included. This allows the client to update the preedit overlay without receiving full terminal grid state.
...
2. FrameUpdate with `preedit.active=false` and `num_dirty_rows=0` — preedit overlay cleared
