...
test "spec: pane split -- splits leaf into split node with two children" {
    ...
    switch (tree[0].?) {
        .split => |s| {
            ...
            try std.testing.expect(s.ratio == 5000);
        },
...
test "spec: pane split -- ratio is stored in split node" {
    ...
    var tree = split_tree.initSingleLeaf(0);
    try split_tree.splitLeaf(&tree, 0, .horizontal, 3000, 1);
    switch (tree[0].?) {
        .split => |s| try std.testing.expect(s.ratio == 3000),
        .leaf => return error.TestUnexpectedResult,
    }
}
...
test "spec: pane equalize -- all split ratios set to 5000 (50%)" {
    // protocol 03-session-pane-management (equalize splits request/response):
    // EqualizeSplitsRequest sets all split ratios to 5000 (50%).
    var tree = split_tree.initSingleLeaf(0);
    try split_tree.splitLeaf(&tree, 0, .horizontal, 3000, 1);
    // Create a nested split on the left child.
    try split_tree.splitLeaf(&tree, split_tree.leftChild(0), .vertical, 7000, 2);

    // Verify ratios are NOT 5000 (50%) initially.
    switch (tree[0].?) {
        .split => |s| try std.testing.expect(s.ratio == 3000),
        .leaf => return error.TestUnexpectedResult,
    }
    ...
    // Verify all splits now have ratio 5000 (50%).
    for (tree) |node_opt| {
        if (node_opt) |node| {
            switch (node) {
                .split => |s| try std.testing.expect(s.ratio == 5000),
                .leaf => {},
            }
        }
    }
...
test "spec: pane tree depth -- max depth enforced on split" {
    ...
    while (depth_reached < 20) : (depth_reached += 1) {
        const result = split_tree.splitLeaf(&tree, current_idx, .horizontal, 5000, slot);
        if (result) |_| {
...
test "spec: pane split -- LayoutChanged broadcast after SplitPaneRequest" {
    ...
    // Split the root pane.
    const new_slot = try entry.allocPaneSlot();
    try split_tree.splitLeaf(tree, 0, .horizontal, 5000, new_slot);
    try std.testing.expectEqual(@as(u8, 2), split_tree.leafCount(tree));
