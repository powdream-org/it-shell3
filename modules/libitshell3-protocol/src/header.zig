...
test "header encode/decode round-trip" {
    ...
    try std.testing.expectEqual(original.payload_len, decoded.payload_len);
...
test "header encode/decode all flag combinations" {
    const cases = [_]Flags{
        .{ .encoding = .binary },
        .{ .response = true },
        .{ .@"error" = true },
    ...
    for (cases) |flags| {
        const hdr = Header{ .msg_type = 0x0002, .flags = flags, .payload_len = 0, .sequence = 1 };
        var buf: [HEADER_SIZE]u8 = undefined;
        hdr.encode(&buf);
        const decoded = try Header.decode(&buf);
...
test "header decode reserved field non-zero" {
    const hdr = Header{ .msg_type = 0x0001, .flags = .{}, .payload_len = 0, .sequence = 1 };
    var buf: [HEADER_SIZE]u8 = undefined;
    hdr.encode(&buf);
...
test "header decode payload too large" {
    const hdr = Header{ .msg_type = 0x0001, .flags = .{}, .payload_len = MAX_PAYLOAD_SIZE + 1, .sequence = 1 };
    var buf: [HEADER_SIZE]u8 = undefined;
    hdr.encode(&buf);
...
test "header decode payload exactly at limit" {
    const hdr = Header{ .msg_type = 0x0001, .flags = .{}, .payload_len = MAX_PAYLOAD_SIZE, .sequence = 1 };
    var buf: [HEADER_SIZE]u8 = undefined;
    hdr.encode(&buf);
    const decoded = try Header.decode(&buf);
    try std.testing.expectEqual(MAX_PAYLOAD_SIZE, decoded.payload_len);
}

test "header sequence number 0 is valid in header" {
    const hdr = Header{ .msg_type = 0x0001, .flags = .{}, .payload_len = 0, .sequence = 0 };
    var buf: [HEADER_SIZE]u8 = undefined;
    hdr.encode(&buf);
    const decoded = try Header.decode(&buf);
